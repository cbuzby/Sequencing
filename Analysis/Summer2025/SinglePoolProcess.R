...
testrawdata %>% select(CHROM, POS, ALT) %>% separate(ALT, into = c("First", "Second"), sep = ",") %>% 
  na.omit() -> exclude
...
exclude %>% left_join(.,ChromKey) %>% select(-CHROM) %>% mutate(CHROM = chromosomes) %>% 
  select(-chromosomes) -> exclude
...
rawdata_called %>% mutate(locus = paste(CHROM, POS)) %>% filter(locus %in% exclKEY$locus == FALSE) %>% 
  select(-locus) -> rawdata_called_ex
...
rawdata_called_ex %>% left_join(Means) %>% ungroup() %>% na.omit() %>%
  group_by(Dataset, CHROM, POS) %>% 
