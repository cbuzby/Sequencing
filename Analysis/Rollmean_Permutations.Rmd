...
CSSI_Zeocin_rollmean_permuted %>% group_by(CHROM, POS) %>% summarize(glm = glmfixed_rep(
                                                                      ...
                                                                      label = c("intercept", "Bulk", "Parent", "BxP_Interaction", "replicate")) -> test5


test5 %>% filter(CHROM != "M", CHROM != "I") %>% ggplot(aes(x = glm, color = label)) + geom_density(size = 1) + ggtitle("All Zeocin Permuted")
test5 %>% filter(CHROM == "III") %>% ggplot(aes(x = glm, color = label)) + geom_density(size = 1) + facet_wrap(~CHROM) + ggtitle("Chrom III Distributions")

test5 %>% filter(CHROM != "M") %>% ggplot(aes(x = glm, color = label)) + geom_density(size = 1) + facet_wrap(~CHROM) + ggtitle("Zeocin Permuted Distributions by Chromosome")

test5 %>% filter(CHROM != "M") %>% ggplot(aes(x = glm, y = label, color = label)) + geom_boxplot() + facet_wrap(~CHROM) + ggtitle("Zeocin Permuted Distributions by Chromosome")
