...
DiluteBSA_smoothed_u %>% filter(CHROM != "I", CHROM != "VI", CHROM != "XI", CHROM != "V") %>% pivot_longer(c(CycZeo_Zprime, CuSO4_Zprime, Fluc_Zprime), names_to = "Experiment", values_to = "Z_Prime") %>%
  ggplot(aes(x = POS, y = Z_Prime, color = Experiment)) + geom_line(size = 2) + 
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + ylim(-5,5) +
  theme_minimal() + #scale_color_manual(values = mylabelcolorsd) + 
      theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) + 
...
DiluteBSA_smoothed_u %>% filter(CHROM != "I", CHROM != "VI", CHROM != "XI", CHROM != "V")%>% pivot_longer(c(CycZeo_Z, CuSO4_Z, Fluc_Z, MissingData), names_to = "Experiment", values_to = "Z_Score") %>%
  ggplot(aes(x = POS, y = Z_Score, color = Experiment)) + geom_point(size = 0.8, alpha = 0.3) + 
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + 
  theme_minimal() + #scale_color_manual(values = mylabelcolorsd) + 
      theme(legend.position = "bottom", axis.text.x=element_blank(),
...
DiluteBSA_smoothed_u %>% pivot_longer(c(CycZeo_Z, CuSO4_Z, Fluc_Z, MissingData), names_to = "Experiment", values_to = "Z_Score") %>%
  ggplot(aes(x = POS, y = Z_Score, color = Experiment)) + geom_point(size = 0.8, alpha = 0.3) + 
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + ylim(-5,5) +
  theme_minimal() + #scale_color_manual(values = mylabelcolorsd) + 
      theme(legend.position = "bottom", axis.text.x=element_blank(),
...
DiluteData$CycZeo = ifelse(DiluteData$Study == "CycZeo", 1, 0)
DiluteData$CuSO4 = ifelse(DiluteData$Study == "CuSO4", 1, 0)
DiluteData$Fluc = ifelse(DiluteData$Study == "Fluc", 1, 0)
...
#####!!!!!!!!!!!!!!!!!!!!########################
# MERGE DATA TOGETHER - this needs to be changed to keep all data

# Dilute_BSA_CycZeo2 %>% select(-'(Intercept)') %>% merge(Dilute_BSA_CuSO42) %>% 
#   select(-'(Intercept)') %>% merge(Dilute_BSA_Fluc2) %>% 
#   select(-'(Intercept)') -> Dilute_BSA_byParent_2

# RUN SMOOTHING
# DiluteBSA_smoothed_2 <- foreach(i=unique(Dilute_BSA_byParent_2$CHROM), .combine=rbind) %dopar%{
#   Dilute_BSA_byParent_2 %>% cybrSmoothBSAWindows2(chr = i)
# }

DiluteDataCounts_2 %>% pivot_longer(c(cz_length, cu_length, f_length), names_to = "Type", values_to = "Counts") %>%
  filter(Counts < 2) %>% select(CHROM, POS) %>% mutate(MissingData = 0) -> dc2
# Merging instead once cybrSmoothBSAWindows2 is run
Dilute_BSA_CuSO42 %>% select(-'(Intercept)_Z', -'(Intercept)_Zprime', -Index) %>% 
  ...
  full_join(dc2) %>% 
  pivot_longer(c(CycZeo_Zprime, Fluc_Zprime, CuSO4_Zprime, MissingData), names_to = "Experiment", values_to = "Z_Prime") %>%
  pivot_longer(c(CycZeo_Z, Fluc_Z, CuSO4_Z), names_to = "ExperimentZ", values_to = "Z_Score") %>%
  distinct() -> DiluteBSA_smoothed_2


DiluteBSA_smoothed_2$CHROM <- factor(DiluteBSA_smoothed_2$CHROM, levels = ChromKey$chromosomes)


save(DiluteBSA_smoothed_2, file = "DiluteBSA_smoothed_2.Rdata")
```
