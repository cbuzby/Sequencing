...
BSA_GLM_fixed <-  function(lrP, chr = "II", windowsize = 100, a = 0){
  ...
  #Run the windows on that chromosome
  WResult <- foreach(i = seq(from = (windowsize+a), to = length(Results$POS), by = windowsize), .combine=rbind) %dopar% {
            #save the median value from original index 1 to original end index
                res_int <- mean(Results$Interaction[(i):(i+windowsize)])
                res_bulk <- mean(Results$bulkHigh[(i):(i+windowsize)])
                res_parent <- mean(Results$parentWine[(i):(i+windowsize)])
                #print CHROM, index, POS, and median
...
```{r}
saveaddition <- list()
for(addition in 1:200){
  saveaddition[[addition]] <- BSA_GLM_fixed(logregParental, chr = "VIII", a = addition, windowsize = 200)
}
