...
pb_VII <- list()
for(i in 1:as.numeric(args[2])){ #PICK NUMBER OF SAMPLES
  pb_VII[[i]] <- pois_binom_sample(lrPbulksums_p, chr = args[1], seedt = i) #PICK CHROMOSOME
}

pb_df <- bind_rows(pb_VII, .id = "Index") #PICK NAME: as.numeric(args[2])

filename <- paste("pb_df", args[1], args[2],".Rdata", sep = "_")
save(pb_df, file = filename)
