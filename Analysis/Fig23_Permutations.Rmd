...
#FUNCTION
permute_glm <- function(rollData, size = 100, parentvector = c("Oak1", "Wine1"), repvector = c("A","B","C","D", "E")){
  ...
  for(i in 1:size){
    ...
    savedatahere <- coefficients(glm(formula = "Allele ~ POS*Parent + Rep", data = as.data.frame(test), weights = ReadCount, family = binomial))
...
Cu_1_B_perm <- permute_glm(rollData = cua1_rolldata, parentvector = c("Oak", "Wine"), repvector = c("A", "B", "C", "D"), size = 500)
...
save(Cutoffs_perm, file = "NewCutoffs_perm.rds")
