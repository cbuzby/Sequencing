...
Epistatic_GLM %>% ggplot(aes(x = POS, y = Summary, color = Factor)) + geom_point()

Epistatic_GLM %>% pivot_wider(names_from = Factor, values_from = Summary) %>% ggplot(aes(x = Bulk, y = Intercept)) + geom_point()
Epistatic_GLM %>% pivot_wider(names_from = Factor, values_from = Summary) %>% ggplot(aes(x = Parent, y = Interaction)) + geom_point()
