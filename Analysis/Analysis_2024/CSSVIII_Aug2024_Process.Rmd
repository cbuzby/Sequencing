...
CuSO4_CSS1_glmer_1byRep %>% 
  ...
  ggplot(aes(x = POS, y = abs(zscore), color = Selection)) + geom_line(size = 0.5, alpha = 0.8) + 
...
Loading in previous cyclohex and zeocin data to compare

```{r, fig.height=3, fig.width=10}
Cyclohex_CSSI_prev <- readRDS("C:/Users/Cassandra/Documents/R_FileTransfer/StandardGLM/HKTMWDRX2_Cyc_CSS1_glm_all_noreps.rds") %>% mutate(Selection = "Cyclohex")
Zeocin_CSSI_prev <- readRDS("C:/Users/Cassandra/Documents/R_FileTransfer/StandardGLM/HKTMWDRX2_Zeo_CSS1_glm_all_noreps.rds") %>% mutate(Selection = "Zeo")

CSS1_Aug2024_glm %>% ungroup() %>%
  select(CHROM, POS, Summary = zscore, Factor = label, Selection) %>% 
  filter(Selection == "Cycloheximide") %>%
  rbind(Cyclohex_CSSI_prev) %>%
  rbind(Zeocin_CSSI_prev) -> cyclo_comp

cyclo_comp %>% filter(Factor %in% c("Background", "Parent")) %>% ggplot(aes(x = POS, y = Summary, color = Selection), alpha = 0.5) +
  geom_line() +
