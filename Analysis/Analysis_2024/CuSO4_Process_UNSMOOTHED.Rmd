...
cbyr_ksmooth <- function(z, p){
  result <- smoothr::smooth_ksmooth(matrix(c(z,p), ncol = 2))
  return(data.frame(result))
}

AllCuSO4_g200 %>% arrange(POS) %>% group_by(CSS, CHROM, Label) %>%
  summarize(cbyr_ksmooth(z = zscore, p = POS)) -> AllCuSO4_Smoothed
