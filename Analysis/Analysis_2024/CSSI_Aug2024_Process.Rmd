...
```{r}
smoothdata_named %>% na.omit() %>%
  mutate_if(is.character, as.factor) -> rd_factor

contrasts(rd_factor$Parent) <- matrix(c(0.5, -0.5))
contrasts(rd_factor$Bulk) <- matrix(c(1, 0))

saveRDS(rd_factor, "CSSI_A24_rd_factorG200.rds")
```
...
```{r}
rd_factor <- readRDS("CSSI_A24_rd_factorG200.rds")

rd_factor %>% mutate(SmoothCount = floor(SmoothCount)) -> rd_factor
```
...
rd_factor %>% filter(CHROM != "M", CHROM != "I") %>% distinct() %>% 
  group_by(CHROM, POS, Day) %>% summarize(zscore = glm_cb2_short(Bulk = Bulk,  
                                                            ...
                                                            formula = "Allele ~ Bulk*Background"),
                                                                  label = c("cept","Bulk", "Background", "Interaction"))  -> Aug24_CSS1_glm_1byRep

saveRDS(Aug24_CSS1_glm_1byRep, file = "Aug24_CSS1_glm_1byRep.rds")

Aug24_CSS1_glm_1byRep %>% na.omit()
```


# Load in post glm/glmer
```{r}
Aug24_CSS1_glmer_1byRep <- readRDS("Aug24_CSS1_glmer_1byRep.rds")
Aug24_CSS1_glm_1byRep <- readRDS("Aug24_CSS1_glm_1byRep.rds")
