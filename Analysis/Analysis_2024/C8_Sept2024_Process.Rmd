...
myfile <- "C:/Users/Cassandra/Documents/GitHub/Sequencing/Analysis/Data/C8_HNGLVDRXY.SortedCat.vcf.output.table"
...
saveRDS(rawdata, file = "CSSI_C8_rawdataG.rds")
...
saveRDS(rawdata_called_ex, file = "CSSI_C8_rawdata_called_ex.rds")
```

```{r}

length(unique(rawdata$POS))
length(unique(pSNPs$POS[pSNPs$CHROM == "VIII"]))

merge(rawdata$POS,pSNPs$POS[pSNPs$CHROM == "VIII"])

rawdata %>% ggplot(aes(x = POS, y = abs(log(as.numeric(AD.REF)/as.numeric(AD.ALT))), color = Dataset)) + geom_point() +
...
rawdata_called_ex <- readRDS("CSSI_C8_rawdata_called_ex.rds")
...
saveRDS(rawdata_smoothed_G200, file = "CSSI_C8_rawdata_smoothed_G200.rds")


```

I fucked up somehow so let's just look at these?

```{r}
rawdata_called_ex %>% ggplot(aes(x = POS, y = Coverage, color = Dataset)) + geom_point()
```
...
# rawdata_called_ex <- readRDS("CSSI_C8_rawdata_smoothed_G200.rds")
rawdata_smoothed_G200 <- readRDS("CSSI_C8_rawdata_smoothed_G200.rds")
...
saveRDS(rd_factor_cyclohex, "CSSI_C8_rd_factor_cyclohex.rds")
saveRDS(rd_factor_ethanol, "CSSI_C8_rd_factor_ethanol.rds")
saveRDS(rd_factor_H2O2, "CSSI_C8_rd_factor_H2O2.rds")
...
```{r}
rd_factor_cyclohex <- readRDS("CSSI_C8_rd_factor_cyclohex.rds") 
rd_factor_ethanol <- readRDS("CSSI_C8_rd_factor_ethanol.rds")
rd_factor_H2O2 <- readRDS("CSSI_C8_rd_factor_H2O2.rds")
```
