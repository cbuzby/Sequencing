...
```{r, fig.width=8, fig.height=3}
#Make a band for underneath
AllCuSO4_glmer_1byRep %>%
  #filter(abs(zscore) > 1.1) %>%
  filter(label == "Bulk") %>%
  ggplot(aes(x = POS)) + 
  geom_vline(aes(xintercept = POS, color = zscore, alpha = abs(zscore) > 1.1)) +
  facet_grid(CSS~CHROM, space = "free", scales = "free") +
  scale_color_gradient2(low = "red", mid = "white", high = "blue") +
  scale_alpha_manual(values = c(0,1)) +
  theme(legend.position = "none", panel.grid = element_blank()) -> Fig3Panel
...
# Fig3
# 
# ggsave("Fig3.tif", plot = Fig3, dpi = 300, width = 8, height = 3)
# ggsave("Fig3.png", plot = Fig3, dpi = 300, width = 8, height = 3)

#plot_grid(Fig3, Fig3Panel, nrow = 2, rel_heights = c(4,2))
```
...
AllCuSO4_glmer_1byRep %>%
  ...
  geom_ribbon(aes(ymin = -Inf, ymax = -0.5), fill = "black") +
  ...
  #               color = abs(zscore))) +
geom_tile(data = AllBulkPeaks[AllBulkPeaks$CSS == "I",], aes(x = POS, y = -1, width = 2, height = 1), color = "cyan") +
  geom_tile(data = AllBulkPeaks[AllBulkPeaks$CSS == "VIII",], aes(x = POS, y = -2, width = 2, height = 1), color = "cyan") +
  facet_grid(~CHROM, space = "free", scales = "free") +
  theme_bw() +
  ylim(-3, 13)+
  scale_color_gradient2(low = "gray", high = "black") +
  scale_alpha_manual(values = c(0,1)) +
...
Trying this with an actual peak caller
...
AllCuSO4_glmer_1byRep %>%
  ...
  na.omit() %>%
  arrange(CHROM, POS) %>%
  mutate(threshold = abs(zscore) > q5) -> testwhatever

equivalent <- function(x){
  x[1] == x[2]
}

testwhatever %>% group_by(CHROM, CSS) %>%
  summarize(POS, zscore, threshold, change = frollapply(threshold, FUN = equivalent, align = "left", n = 2)) -> anothertest

for(i in unique(AllCuSO4_glmer_1byRep$CHROM)){
  AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == i) %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  ggplot(aes(x = POS, y = zscore, color = CSS)) + geom_point() +
    ggtitle(i) +
    theme_bw() -> testplot
...
AllCuSO4_glmer_1byRep %>%
  ...
  filter(abs(zscore) > q5) %>%
  group_by(CSS) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "I") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS) -> C1

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "II") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 2e05) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max)%>%
  mutate(CHROM = "II") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS) -> C2
...
AllCuSO4_glmer_1byRep %>%
  ...
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "III") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C3

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "IV") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 400000, POS > 1000000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "IV")%>%
  ungroup() %>% select(CHROM, POS, zscore, CSS) -> C4

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "IX") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "IX") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C9

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "V") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 400000, POS > 1000000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "V")%>%
  ungroup() %>% select(CHROM, POS, zscore, CSS) -> C5

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "VI") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 150000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "VI") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C6

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "VII") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 650000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "VII") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C7

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "VIII") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "VIII") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C8

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "X") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 650000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "X") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C10

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "XI") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 400000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "XI") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C11

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "XII") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 500000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "XII") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C12

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "XIII") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 200000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "XIII")%>%
  ungroup() %>% select(CHROM, POS, zscore, CSS) -> C13


AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "XIV") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 300000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "XIV") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C14

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "XV") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 300000, POS > 750000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "XV") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C15

AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk", CHROM == "XVI") %>%
  merge(q5_all) %>%
  filter(abs(zscore) > q5) %>%
  group_by(CSS, POS > 700000) %>%
  summarize( POS, zscore, max = max(abs(zscore))) %>%
  filter(abs(zscore) == max) %>%
  mutate(CHROM = "XVI") %>%
  ungroup() %>% select(CHROM, POS, zscore, CSS)-> C16


rbind(C1, C2, C3, C4, C5, C6, C7, C8,
      C9, C10, C11, C12, C13, C14, C15, C16) -> AllBulkPeaks

AllBulkPeaks %>% mutate(POS = POS+10) %>% rbind(AllBulkPeaks)%>%
  mutate(POS = POS-10) %>% rbind(AllBulkPeaks) -> AllBulkPeaks_wide

AllBulkPeaks_wide %>% arrange(CSS, CHROM, POS, zscore) -> Additive_Peaks
write.csv(Additive_Peaks, file = "Additive_Peaks.csv", row.names = FALSE)
...
temppc %>% 
  ...
  summarize(POS = POS,
            abs_zscore = abs_zscore, 
            smooth_abs_z = frollapply(abs_zscore, mean, n = 200, align = "center")) %>%
  na.omit() %>% 
  group_by(CHROM, CSS) %>% arrange(POS) %>%
  summarize(POS = POS, abs_zscore = abs_zscore, smooth_abs_z = smooth_abs_z,
            slope = frollapply(smooth_abs_z, FUN = slope_change, n = 300, align = "center")) %>%
...
#   geom_point(data = peaks, aes(x = POS, y = zscore)) + 
#   # geom_vline(data = tempStartEnd, aes(xintercept = Start), color = "black") +
#   # geom_vline(data = tempStartEnd, aes(xintercept = End), linetype = "dashed", color = "gray") +
#   facet_grid(CSS~CHROM, scales = "free", space = "free") +
...
AllCuSO4_glmer_1byRep %>%
  ...
  geom_tile(data = AllCuSO4_glmer_1byRep[AllCuSO4_glmer_1byRep$CSS == "VIII" & AllCuSO4_glmer_1byRep$label == "Bulk"& abs(AllCuSO4_glmer_1byRep$zscore) > 1.116373,],
            aes(x = POS, y = -2, width = 1, height = 1,
                color = abs(zscore))) +
geom_tile(data = AllBulkPeaks_wide[AllBulkPeaks_wide$CSS == "I",], aes(x = POS, y = -1, width = 2, height = 1), color = "cyan") +
  geom_tile(data = AllBulkPeaks_wide[AllBulkPeaks_wide$CSS == "VIII",], aes(x = POS, y = -2, width = 2, height = 1), color = "cyan") +
  facet_grid(~CHROM, space = "free", scales = "free") +
  theme_bw() +
  ylim(-3, 13)+
  scale_color_gradient2(low = "gray", high = "black") +
...
AllCuSO4_glmer_1byRep %>%
  ...
  geom_ribbon(aes(ymin = -Inf, ymax = -0.25), fill = "white") +
  #geom_ribbon(aes(ymin = -Inf, ymax = -0.5), fill = "black") +
  geom_point(data = peaks[peaks$CSS == "I",], aes(x = POS, y = zscore), color = "darkturquoise", alpha = 0.5) + 
  geom_point(data = peaks[peaks$CSS == "VIII",], aes(x = POS, y = zscore), color = "blue", alpha = 0.5) + 
...
geom_tile(data = peaks[peaks$CSS == "I",], aes(x = POS, y = -1, width = 2, height = 1), color = "darkturquoise") +
  ...
  facet_grid(~CHROM, space = "free", scales = "free") +
  theme_bw() +
  scale_color_gradient2(low = "gray80", high = "gray40") +
  scale_alpha_manual(values = c(0,1)) +
