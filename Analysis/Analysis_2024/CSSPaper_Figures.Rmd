...
AllCuSO4_glmer_1byRep %>%
  filter(label == "Interaction", CHROM == "XII") %>%
  ungroup() %>%
  group_by(CHROM, CSS) %>%
  summarize(max_zscore = max(zscore),
            min_zscore = min(zscore)) %>%
...
AllCuSO4_glmer_1byRep %>%
  ...
  theme(legend.position = "none",panel.grid = element_blank()) +
   theme(strip.background.x = element_rect(color = "skyblue3", fill = c("skyblue3", "darkturquoise" )),
...
AllCuSO4_glmer_1byRep %>%
  ...
  theme(legend.position = "none",panel.grid = element_blank()) +
  scale_linetype_manual(values = c(2,1)) +
  scale_color_manual(values = c(1,"red","red","red","red","red","red",1,
...
AllCuSO4_glmer_1byRep %>%
  ...
  geom_line() +
  geom_hline(data = q5_Bulk, aes(yintercept = Bulkq5, linetype = CSS), size = 0.5) +
  geom_vline(data = peaks_rainbow, aes(xintercept = POS, color = CSS), alpha = 0.3, size = 1) + 
  facet_grid(~CHROM, space = "free", scales = "free") +
  theme(strip.background.x = element_rect(color = "black", fill = c("black", "darkturquoise" )),
        strip.text.x = element_text(color = "white", size = 10, face = "bold")) +
  #theme_classic() +
  theme(legend.position = "none",panel.grid = element_blank()) +
  scale_color_manual(values = c("skyblue3", "darkorange"))
```
...
AllCuSO4_glmer_1byRep %>%
  filter(label == "Bulk") %>%
  ggplot(aes(x = POS, y = abs(zscore), alpha = CSS)) + 
  geom_line() +
  geom_hline(data = q5_Bulk, aes(yintercept = Bulkq5, alpha = CSS), size = 0.5) +
  geom_vline(data = peaks_rainbow, aes(xintercept = POS, color = CHROM, linetype = CSS), alpha = 0.4, size = 1) + 
  facet_grid(~CHROM, space = "free", scales = "free") +
  theme_bw() +
  theme(#strip.background.x = element_rect(color = "black", fill = c("black", "gray" )),
        #strip.text.x = element_text(color = "white", size = 10, face = "bold"),
  ...
  scale_color_manual(values = c(1,"darkorange","darkorange","darkorange","darkorange","darkorange","darkorange",1,
...
#REDOING PA TO GET RID OF THE OTHER HALF
rd_factor %>% filter(POS %in% IntPeaks2$POS) %>%
  mutate(Background = gsub("Oak", 0, Background),
         Background = gsub("Wine", 1, Background)) %>%
  mutate(Bulk = gsub("Dilute", "Unselected", Bulk)) %>%
  ...
  scale_fill_manual(values = c("#177871", "gray")) +
  scale_color_manual(values = c("#177871", "gray")) +
    xlab("OAK <- Parental Background -> WINE") +
  ylab("Wine Allele Frequency") +
    theme(legend.position = "bottom", panel.grid.minor = element_blank(), panel.grid.major = element_blank()) +
  #scale_x_continuous( breaks=pretty_breaks()) +
    # geom_hline(yintercept = Inf, color = "white", size = 4) +
    theme(strip.background.y = element_rect(fill = c("black", "#196761" )),
        strip.text.y = element_text(color = "white", size = 10, face = "bold"))+
  facet_grid( ~ paste(CHROM, POS)) -> pa
...
rd_factor %>% filter(POS %in% IntPeaks2$POS) %>%
  ...
  theme_bw() +
    theme(legend.position = "bottom", 
          ...
          axis.ticks.x = element_blank(),
          strip.background = element_rect(fill = c("darkorange", "#CD5C5C" )),
...
AllCuSO4_glmer_1byRep %>% merge(IntRange2) %>%
  ...
  xlab(paste("Peak Position +/-",chromwidth/1000,"kb")) + ylab("Z-score") +
...
AllCuSO4_glmer_1byRep %>% merge(IntRange2) %>%
  ...
  scale_shape_manual(values = c(0,2)) +
  scale_alpha_manual(values = c(0, 0.8)) +
  scale_color_manual(values = c("black", "#7a0c86")) + 
  theme_bw() +
  theme(legend.position = "none",
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.border = element_rect(color = "gray")) -> pb
```

```{r}
rd_factor %>% filter(POS %in% IntPeaks2$POS) -> IntCounts
...
AllCuSO4_glmer_1byRep %>%
  ...
  geom_hline(aes(yintercept = q5)) +
  scale_color_manual(values = c("black","cyan3","orange","hotpink","violet", "#7a0c86")) +
    facet_grid(CSS~CHROM, scales = "free", space = "free") +
  ylim(0,11) +
  #theme_bw() +
  theme(strip.background.x = element_rect(fill = c("gray"),color = c("gray")),
        ...
        axis.ticks.x=element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) #-> pc
...
AllCuSO4_glmer_1byRep %>%
  ...
  geom_vline(data = IntPeaks,
             aes(xintercept = POS),
             color = "darkorange", linetype = "dashed", size = 1, alpha = 0.4)+
  xlab("") + ylab("abs Z-score") +
  geom_hline(aes(yintercept = q5)) +
  scale_color_manual(values = c("black", "purple")) +
    facet_grid(CSS~CHROM, scales = "free", space = "free") +
...
plot_grid(pc, pa, pb, ncol = 1, rel_heights = c(3,2,1.5))
...
IntCounts %>% group_by(Dataset, CHROM, POS) %>%
  summarize(sumCount = sum(SmoothCount)) %>%
  merge(IntCounts) %>%
  mutate(Freq = SmoothCount/sumCount) %>%
  mutate(Background = gsub("Oak", 0, Background),
         Background = gsub("Wine", 1, Background)) %>%
  mutate(Bulk = gsub("Dilute", "Unselected", Bulk)) %>%
  ggplot(aes(x = as.numeric(Background), y = Freq, color = Allele)) + 
  geom_point(shape = 1, size = 2, stroke = 1, alpha = 0.8) + 
  geom_smooth(method = "lm", alpha = 0.2) +
  scale_color_manual(values = c("palegreen3", "firebrick3")) +
    xlab("Parental Background") +
...
IntPeaks2 %>% group_by(CHROM, POS) %>%
  summarize(POSNew = seq(POS - chromwidth, POS + chromwidth, length.out = 1+chromwidth*2)) %>%
  mutate(POS_rel = POSNew - POS) %>%
  select(CHROM, POS = POSNew, Rel = POS_rel) -> IntRange
  
AllCuSO4_glmer_1byRep %>% merge(IntRange) %>%
  filter(label %in% c("Bulk", "Interaction"), CSS == "VIII") %>%
  mutate(Coefficient = label) %>%
  #filter(abs(zscore) > 1.1) %>%
  ggplot(aes(x = Rel, y = abs(zscore), color = Coefficient, size = Rel == 0, shape = Rel == 0, alpha = Rel == 0)) + 
  geom_point(stroke = 1) + 
  #geom_vline(data = IntPeaks2, aes(xintercept = 0, linetype = factor(POS)), color = "red", linetype = "dashed", size = 1, alpha = 0.4) +
  facet_grid( ~ CHROM, scales = "free") +
  ylim(0,10) +
  xlim(-chromwidth, chromwidth) +
  xlab("") + ylab("abs Z-score") +
  scale_size_manual(values = c(0.1,2)) +
  scale_shape_manual(values = c(0,2)) +
  ...
  scale_color_manual(values = c("black", "purple")) +
  theme(legend.position = "bottom") +
  ggtitle(paste("Interaction peaks +/-", chromwidth/1000, "kb"))
...
AllCuSO4_glmer_1byRep %>% merge(IntRange) %>%
  ...
  geom_point(stroke = 1) + 
  facet_grid( ~ CHROM, scales = "free") +
  ylim(0,10) + xlim(-chromwidth, chromwidth) +
  xlab("Peak Position +/- 100 kb") + ylab("abs Z-score") +
  
  scale_size_manual(values = c(0.1,2)) +
  scale_shape_manual(values = c(0,2)) +
  scale_alpha_manual(values = c(0.01, 0.8)) +
  scale_color_manual(values = c("black", "purple")) + 
  theme(legend.position = "none") -> pb
...
AllCuSO4_glmer_1byRep %>% merge(IntRange2) %>%
  ...
  facet_grid( ~ CHROM, scales = "free", space = "free_x") +
  #ylim(-10,10) + #xlim(-chromwidth, chromwidth) +
  xlab("Peak Position +/- 500 kb") + ylab("Z-score") +
...
New version of the intercrosses

```{r, eval = FALSE}
AllCuSO4_glmer_1byRep %>% filter(CHROM == "VII", label == "Interaction")  %>% arrange(desc(abs(zscore))) %>% head(1) -> a
AllCuSO4_glmer_1byRep %>% filter(CHROM == "VII", label == "Interaction")  %>% filter(POS > 204728) %>% arrange(desc(abs(zscore))) %>% head(1) -> b
AllCuSO4_glmer_1byRep %>% filter(CHROM == "VII", label == "Interaction")  %>% filter(POS > 4e05) %>% arrange(desc(abs(zscore))) %>% head(1) -> c2
AllCuSO4_glmer_1byRep %>% filter(CHROM == "VII", label == "Interaction")  %>% filter(POS > 4e05 & POS < 5e05) %>% arrange(desc(abs(zscore))) %>% head(1) -> b2

AllCuSO4_glmer_1byRep %>% filter(CHROM == "XV", label == "Interaction")  %>% arrange(desc(abs(zscore))) %>% head(1) -> c
AllCuSO4_glmer_1byRep %>% filter(CHROM == "XIV", label == "Interaction")  %>% arrange(desc(abs(zscore))) %>% head(1) -> d
AllCuSO4_glmer_1byRep %>% filter(CHROM == "XII", label == "Interaction")  %>% arrange(desc(abs(zscore))) %>% head(1) -> e

AllCuSO4_glmer_1byRep %>% filter(CHROM == "XI", label == "Interaction")  %>% arrange(desc(abs(zscore))) %>% head(1) -> f
AllCuSO4_glmer_1byRep %>% filter(CHROM == "XI", label == "Interaction")  %>% filter(POS < 500000) %>% arrange(desc(abs(zscore))) %>% head(1) -> g
AllCuSO4_glmer_1byRep %>% filter(CHROM == "X", label == "Interaction")  %>% arrange(desc(abs(zscore))) %>% head(1) -> h
AllCuSO4_glmer_1byRep %>% filter(CHROM == "X", label == "Interaction")  %>% filter(POS > 224728) %>% arrange(desc(abs(zscore))) %>% head(1) -> i
AllCuSO4_glmer_1byRep %>% filter(CHROM == "X", label == "Interaction")  %>% filter(POS > 360000) %>% arrange(desc(abs(zscore))) %>% head(1) -> j

rbind(a,b,c,d,e,f,g,h,i,j) %>%
  rbind(c2, b2) %>%
  ungroup() %>%
  select(-CSS) %>%
  mutate(Sig = abs(zscore) > 1.1) %>%
  mutate(Sig = abs(as.numeric(Sig)-1)) -> IntPeaksAIG

################################################################################

addtoplot %>% mutate(label = "Bulk", CSS = "F12_mp", sdz = NA, CHROM = as.character(as.roman(CHROM))) %>%
  select(CHROM = CHROM, label = label, CSS = CSS, POS = POS, zscore = LOD, sdz = sdz) -> AIGs_combine

Cu_MP %>% mutate(label = "Bulk", CSS = "F2_mp", sdz = NA, CHROM = as.character(as.roman(CHROM))) %>%
  select(CHROM = CHROM, label = label, CSS = CSS, POS = Bin.start..bp., zscore = LOD.score, sdz = sdz) %>%
  rbind(AllCuSO4_glmer_1byRep) %>%
  rbind(AIGs_combine) %>%
  filter(CHROM %in% c("XI", "VII", "XII", "X", "XV", "XIV"), 
         CSS != "I", zscore < 10, label %in% c("Bulk", "Interaction")) %>%
  ggplot(aes(x = POS, y = abs(zscore), color = paste(label,CSS))) + geom_line() +
  geom_vline(data = IntPeaksAIG, aes(xintercept = POS, linetype = factor(Sig)), color = "purple", alpha = 0.4) +
  facet_grid(CSS ~ CHROM, scales = "free", space = "free") +
  ylim(0,10) + ylab(" ") + xlab("Chromosomal Position") +
  scale_color_manual(values = c("salmon","salmon","black", "purple", "gray"))+
  theme(legend.position = "none")  -> aig1

Cu_MP %>% mutate(label = "Bulk", CSS = "F2_mp", sdz = NA, CHROM = as.character(as.roman(CHROM))) %>%
  select(CHROM = CHROM, label = label, CSS = CSS, POS = Bin.start..bp., zscore = LOD.score, sdz = sdz) %>%
  rbind(AllCuSO4_glmer_1byRep) %>%
  rbind(AIGs_combine) %>%
  filter(CHROM %in% c("VII"), 
         CSS != "I", zscore < 10, label %in% c("Bulk", "Interaction")) %>%
  filter(CSS != "F2_mp") %>%
  ggplot(aes(x = POS, y = abs(zscore), color = paste(label,CSS))) + geom_line() +
  geom_vline(data = IntPeaksAIG[IntPeaksAIG$CHROM == "VII",], aes(xintercept = POS, linetype = factor(Sig)), color = "purple", alpha = 0.2, size = 2) +
  facet_grid( ~ CHROM, scales = "free", space = "free") +
  #ylim(0,10) + 
  ylab(" ") + xlab("Chr VII Position (bp)") +
  scale_color_manual(values = c("salmon","black", "purple", "gray"))+
    theme_bw() +   scale_x_continuous( breaks=pretty_breaks()) +
  theme(legend.position = "none")  -> aig2

plot_grid(aig2, aig1, ncol = 1)
...
AllCuSO4_glmer_1byRep %>% filter(CHROM == "III") %>%
  select(-sdz) %>%
  pivot_wider(names_from = CSS, values_from = zscore) %>%
  ggplot(aes(x = I, y = VIII, color = label)) + geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  theme_classic() 

AllCuSO4_glmer_1byRep %>% filter(CHROM == "III", label != "cept") %>%
  select(-sdz) %>%
  pivot_wider(names_from = CSS, values_from = zscore) %>%
  ggplot(aes(x = I, y = VIII, color = label)) + geom_point() +
...
AllCuSO4_glmer_1byRep %>% filter(label == "Bulk", CHROM %in% c("I", "VIII") == FALSE) %>%
  select(-sdz) %>%
  pivot_wider(names_from = CSS, values_from = zscore) %>%
  ggplot(aes(x = abs(I), y = abs(VIII), color = abs(I) > 1.1 & abs(VIII) > 1.1)) + geom_point(alpha = 0.08) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  xlim(0,8) + ylim(0,8) +
  theme_classic() +
...
AllCuSO4_glmer_1byRep %>% filter(label == "Bulk", CHROM %in% c("I", "VIII") == FALSE) %>%
  select(-sdz) %>%
  pivot_wider(names_from = CSS, values_from = zscore) %>%
  ggplot(aes(x = I, y = VIII, color = abs(I) > 1.1 & abs(VIII) > 1.1)) + geom_point(alpha = 0.08) +
  geom_vline(xintercept = 0) +
  geom_hline(yintercept = 0) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
   xlab("Fixed Chr I additive coefficient Z-score") + ylab("Fixed Chr VIII additive coefficient Z-score") +
  #xlim(0,8) + ylim(0,8) +
  theme_classic() +
  scale_color_manual(values = c("gray", "black")) +
  #facet_wrap("CHROM", nrow = 2) +
...
Comparing other interaction peaks

```{r}
#CHROMOSOME VII
AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "VII", label == "Bulk", CSS == "VIII") -> glm7data

AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "VII", label == "Interaction", CSS == "VIII") -> glm7data_i

glm7data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp
glm7data_i %>% filter(POS > 204728) %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp2

Cu_MP %>% filter(CHROM == "VII") %>% 
  ggplot(aes(x = Bin.start..bp., y = LOD.score, color = CHROM)) + geom_line() + 
  geom_line(data = glm7data, aes(x = POS, y = abs(zscore), linetype = CSS), color = "black") +
  geom_line(data = glm7data_i, aes(x = POS, y = abs(zscore), linetype = CSS), color = "purple", size = 1) +
  facet_grid(~CHROM, scales = "free", space = "free")  +
    geom_line(data = addtoplot[addtoplot$CHROM == "VII",], aes(x = POS, y = LOD), color = "red", size = 1, alpha = 0.5) +
  geom_vline(xintercept = peaktemp$POS, alpha = 0.4, size = 1, color = "purple") +
  geom_vline(xintercept = peaktemp2$POS, alpha = 0.4, size = 1, color = "purple") +
...
#CHROMOSOME X
AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "X", label == "Bulk", CSS == "VIII") -> glm10data

AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "X", label == "Interaction", CSS == "VIII") -> glm10data_i

glm10data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp
glm10data_i %>% filter(POS > 224728) %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp2
glm10data_i %>% filter(POS > 360000) %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp3

Cu_MP %>% filter(CHROM == "X") %>% 
  ggplot(aes(x = Bin.start..bp., y = LOD.score, color = CHROM)) + geom_line() + 
  geom_line(data = glm10data, aes(x = POS, y = abs(zscore), linetype = CSS), color = "black") +
  geom_line(data = glm10data_i, aes(x = POS, y = abs(zscore), linetype = CSS), color = "purple", size = 1) +
  # geom_vline(data = peaks[peaks$CHROM == "X" & peaks$label == "Interaction",], aes(Xntercept = peakPOS), linetype = "dashed") +
  ylim(0, 10) +
  geom_hline(yintercept = 1.1) +
  facet_grid(~CHROM, scales = "free", space = "free")  +
    geom_line(data = addtoplot[addtoplot$CHROM == "X",], aes(x = POS, y = LOD), color = "red", size = 1, alpha = 0.5) +
    geom_vline(xintercept = peaktemp$POS, alpha = 0.4, size = 1, color = "purple") +
    geom_vline(xintercept = peaktemp2$POS, alpha = 0.4, size = 1, color = "purple") +
    geom_vline(xintercept = peaktemp3$POS, alpha = 0.4, size = 1, color = "purple") +
  ylab("") + xlab("")+ ggtitle("") +
theme_minimal() +
  theme(legend.position = "none")
```
...
#CHROMOSOME XI
AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XI", label == "Bulk", CSS == "VIII") -> glm11data

AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XI", label == "Interaction", CSS == "VIII") -> glm11data_i

glm11data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp
glm11data_i %>% filter(POS < 500000) %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp2

Cu_MP %>% filter(CHROM == "XI") %>% 
  ggplot(aes(x = Bin.start..bp., y = LOD.score, color = CHROM)) + geom_line() + 
  geom_line(data = glm11data, aes(x = POS, y = abs(zscore), linetype = CSS), color = "black") +
  geom_line(data = glm11data_i, aes(x = POS, y = abs(zscore), linetype = CSS), color = "purple", size = 1) +
  # geom_vline(data = peaks[peaks$CHROM == "XI" & peaks$label == "Interaction",], aes(xintercept = peakPOS), linetype = "dashed") +
  # geom_vline(data = peaks[peaks$CHROM == "XI" & peaks$label == "Bulk",], aes(xintercept = peakPOS)) +
  geom_hline(yintercept = 1.1) +
  facet_grid(~CHROM, scales = "free", space = "free")  +
    geom_line(data = addtoplot[addtoplot$CHROM == "XI",], aes(x = POS, y = LOD), color = "red", size = 1, alpha = 0.5) +
    geom_vline(xintercept = peaktemp$POS, alpha = 0.4, size = 1, color = "purple") +
    geom_vline(xintercept = peaktemp2$POS, alpha = 0.4, size = 1, color = "purple") +
  ylab("") + xlab("")+ ggtitle("") +
theme_minimal() +
...
#CHROMOSOME XII
AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XII", label == "Bulk", CSS == "VIII") -> glm12data

AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XII", label == "Interaction", CSS == "VIII") -> glm12data_i

glm12data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp


Cu_MP %>% filter(CHROM == "XII") %>% 
  ggplot(aes(x = Bin.start..bp., y = LOD.score, color = CHROM)) + geom_line() + 
  geom_line(data = glm12data, aes(x = POS, y = abs(zscore), linetype = CSS), color = "black") +
  geom_line(data = glm12data_i, aes(x = POS, y = abs(zscore), linetype = CSS), color = "purple", size = 1) +
  # geom_vline(data = peaks[peaks$CHROM == "XII" & peaks$label == "Interaction",], aes(XIIntercept = peakPOS), linetype = "dashed") +
  # geom_vline(data = peaks[peaks$CHROM == "XII" & peaks$label == "Bulk",], aes(XIIntercept = peakPOS)) +
  geom_hline(yintercept = 1.1) +
  facet_grid(~CHROM, scales = "free", space = "free")  +
    geom_line(data = addtoplot[addtoplot$CHROM == "XII",], aes(x = POS, y = LOD), color = "red", size = 1, alpha = 0.5) +
    geom_vline(xintercept = peaktemp$POS, alpha = 0.4, size = 1, color = "purple") +
  ylab("") + xlab("")+ ggtitle("") +
  theme_minimal() +
  theme(legend.position = "none")
...
#CHROMOSOME XIV
AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XIV", label == "Bulk", CSS == "VIII") -> glm14data

AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XIV", label == "Interaction", CSS == "VIII") -> glm14data_i

glm14data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp

Cu_MP %>% filter(CHROM == "XIV") %>% 
  ggplot(aes(x = Bin.start..bp., y = LOD.score, color = CHROM)) + geom_line() + 
  geom_line(data = glm14data, aes(x = POS, y = abs(zscore), linetype = CSS), color = "black") +
  geom_line(data = glm14data_i, aes(x = POS, y = abs(zscore), linetype = CSS), color = "purple", size = 1) +
  # geom_vline(data = peaks[peaks$CHROM == "XIV" & peaks$label == "Interaction",], aes(XIVntercept = peakPOS), linetype = "dashed") +
  # geom_vline(data = peaks[peaks$CHROM == "XIV" & peaks$label == "Bulk",], aes(XIVntercept = peakPOS)) +
  geom_hline(yintercept = 1.1) +
  facet_grid(~CHROM, scales = "free", space = "free")  +
    geom_line(data = addtoplot[addtoplot$CHROM == "XIV",], aes(x = POS, y = LOD), color = "red", size = 1, alpha = 0.5) +
    geom_vline(xintercept = peaktemp$POS, alpha = 0.4, size = 1, color = "purple") +
  ylab("") + xlab("")+ ggtitle("") +
  theme_minimal() +
  theme(legend.position = "none")
...
#CHROMOSOME XV
AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XV", label == "Bulk", CSS == "VIII") -> glm15data

AllCuSO4_glmer_1byRep %>%
  filter(CHROM == "XV", label == "Interaction", CSS == "VIII") -> glm15data_i

glm15data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> peaktemp


Cu_MP %>% filter(CHROM == "XV") %>% 
  ggplot(aes(x = Bin.start..bp., y = LOD.score, color = CHROM)) + geom_line() + 
  geom_line(data = glm15data, aes(x = POS, y = abs(zscore), linetype = CSS), color = "black") +
  geom_line(data = glm15data_i, aes(x = POS, y = abs(zscore), linetype = CSS), color = "purple", size = 1) +
  # geom_vline(data = peaks[peaks$CHROM == "XV" & peaks$label == "Interaction",], aes(XVntercept = peakPOS), linetype = "dashed") +
  # geom_vline(data = peaks[peaks$CHROM == "XV" & peaks$label == "Bulk",], aes(XVntercept = peakPOS)) +
  geom_hline(yintercept = 1.1) +
  facet_grid(~CHROM, scales = "free", space = "free")  +
    geom_line(data = addtoplot[addtoplot$CHROM == "XV",], aes(x = POS, y = LOD), color = "red", size = 1, alpha = 0.5) +
    geom_vline(xintercept = peaktemp$POS, alpha = 0.4, size = 1, color = "purple") +
  ylab("") + xlab("")+ ggtitle("") +
  theme_minimal() +
  theme(legend.position = "none")
...
### Trying to make this more streamlined?

```{r}
glm7data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> a
glm7data_i %>% filter(POS > 204728) %>% arrange(desc(abs(zscore))) %>% head(1) -> b
glm15data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> c
glm14data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> d
glm12data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> e

glm11data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> f
glm11data_i %>% filter(POS < 500000) %>% arrange(desc(abs(zscore))) %>% head(1) -> g
glm10data_i %>% arrange(desc(abs(zscore))) %>% head(1) -> h
glm10data_i %>% filter(POS > 224728) %>% arrange(desc(abs(zscore))) %>% head(1) -> i
glm10data_i %>% filter(POS > 360000) %>% arrange(desc(abs(zscore))) %>% head(1) -> j
...
# AllCuSO4_glmer_1byRep %>%
#   filter(label == "Interaction", CSS == "VIII") %>%
#   group_by(CHROM) %>%
#   summarize(zscore = max(abs(zscore))) %>%
#   filter(zscore > 1.1) %>%
#   merge(AllCuSO4_glmer_1byRep) -> IntPeaks
# 
# AllCuSO4_glmer_1byRep %>%
#   filter(label == "Interaction", CSS == "VIII") %>%
#   group_by(CHROM) %>%
...
#   filter(zscore < -1.1) %>%
#   merge(AllCuSO4_glmer_1byRep) %>%
#   rbind(IntPeaks)  %>% select(-CSS) -> IntPeaks
...
Cu_MP %>% mutate(label = "Bulk", CSS = "F2_mp", sdz = NA, CHROM = as.character(as.roman(CHROM))) %>%
  ...
  filter(CHROM %in% c("XI", "VII", "XII", "X", "XV", "XIV"), 
         CSS != "I", zscore < 10, label %in% c("Bulk", "Interaction")) %>%
  ggplot(aes(x = POS, y = abs(zscore), color = paste(label,CSS))) + geom_line() +
  geom_vline(data = IntPeaks, aes(xintercept = POS), color = "purple", alpha = 0.4) +
  facet_grid(CSS ~ CHROM, scales = "free", space = "free") +
  ylim(0,10) +
  scale_color_manual(values = c("salmon","salmon","black", "purple", "gray"))+
  theme(legend.position = "none")  
...
```{r, fig.width=12, fig.height=4}
RM_YPS <- read.table(url("https://storage.googleapis.com/plos-corpus-prod/10.1371/journal.pgen.1002570/1/pgen.1002570.s009.txt?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=wombat-sa%40plos-prod.iam.gserviceaccount.com%2F20240814%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20240814T205418Z&X-Goog-Expires=86400&X-Goog-SignedHeaders=host&X-Goog-Signature="), header = TRUE)

RM_YPS %>% ggplot(aes(x = Position, y = Copper.sulfate)) + geom_point(alpha = 0.1) + facet_grid(~Chromosome, space = "free", scales = "free")
...
# MULTIPOOL but for real


```{r, eval = FALSE}
#THIS HAS NOT BEEN RUN BECAUSE IT ISN'T NEEDED REALLY
...
for(d in unique(CuSO4_mpsetup_final$Bulk)){
  for(c in unique(CuSO4_mpsetup_final$CHROM)){
   CuSO4_mpsetup_final %>% filter(CHROM == c, Bulk == d) %>%
      arrange(POS) %>% ungroup() %>%
      select(POS, Wine, Oak) -> temp
    filename = paste("MultiPool_Inputs/CUSO4all", d,"_", c,"Aug1524.txt", sep = "")
    write.table(temp, file = filename, col.names = FALSE, row.names = FALSE)
  }
}
...
```{r, eval = FALSE}
CB_AIGs_called <- readRDS("CB_AIGs_called.rds")


CB_AIGs_called %>% 
  filter(grepl("_C_", Dataset)) %>%
  pivot_wider(names_from = Allele, values_from = Reads) %>%
  unnest() %>%
  na.omit() -> AIG_C_final

CB_AIGs_called %>% 
  filter(grepl("_U_", Dataset)) %>%
  pivot_wider(names_from = Allele, values_from = Reads) %>%
  unnest() %>%
  na.omit() -> AIG_D_final
...
for(c in unique(AIG_C_final$CHROM)){
   AIG_C_final %>% filter(CHROM == c) %>%
      arrange(POS) %>% ungroup() %>%
      select(POS, Wine, Oak) -> temp
  d <- "Copper"
    filename = paste("MultiPool_Inputs/AIG", d,"_", c,"Aug1524.txt", sep = "")
    write.table(temp, file = filename, col.names = FALSE, row.names = FALSE)
}

for(c in unique(AIG_D_final$CHROM)){
   AIG_C_final %>% filter(CHROM == c) %>%
      arrange(POS) %>% ungroup() %>%
      select(POS, Wine, Oak) -> temp
  d <- "Dilute"
...
```{r}
#Load in all CuSO4 F12 data for r = 2000
AIG_MP <- head(Cu_MP,1)
for(i in as.character(as.roman(1:16))){
  filename <- paste("MultiPool_Inputs/AIG_AUG1524_r2000_", i, ".txt", sep = "")
  tempfile <- read.table(filename, sep = "\t", header = TRUE) %>% mutate(CHROM = i)
  
  AIG_MP <- rbind(AIG_MP, tempfile)
}

AIG_MP %>% ggplot(aes(x = Bin.start..bp., y = MLE.allele.freq.)) + geom_line() + 
  #ylim(0, 0.5) +
  facet_grid(~CHROM, scales = "free", space = "free")

AIG_MP %>% ggplot(aes(x = LOD.score)) + geom_density() +
  theme_classic()
```

```{r}
#Load in all CuSO4 F12 data for r = 200
AIG_MP <- head(Cu_MP,1)
for(i in as.character(as.roman(1:16))){
  filename <- paste("MultiPool_Inputs/AIG_AUG1524_r200_", i, ".txt", sep = "")
  tempfile <- read.table(filename, sep = "\t", header = TRUE) %>% mutate(CHROM = i)
  
  AIG_MP <- rbind(AIG_MP, tempfile)
}

AIG_MP %>% ggplot(aes(x = Bin.start..bp., y = LOD.score)) + geom_line() + 
  ylim(0, 0.5) +
  facet_grid(~CHROM, scales = "free", space = "free")
