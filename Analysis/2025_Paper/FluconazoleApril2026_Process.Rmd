...
CrossoverPoints <- Data %>% mutate(abs_zscore = abs(zscore)) %>% 
        ...
        summarize(POS = POS, abs_zscore = abs_zscore, smooth_abs_z = frollapply(abs_zscore, 
            FUN = mean, n = 1, align = "center")) %>% na.omit() %>% 
...
peaks_8 <- cybr_lmpeaks(Data = Fluconazole_CSS8_glm, cutoff = 1.591914)
saveRDS(peaks_8, file = "Fluc_CSS8_peaks_w700.rds")

peaks_1 <- cybr_lmpeaks(Data = Fluconazole_CSS1_glm, cutoff = 1.591914, width = 900)
saveRDS(peaks_1, file = "Fluc_CSS1_peaks_w900.rds")
```

```{r}
table(CrossoverPoints$CHROM)

CrossoverPoints %>% filter(CHROM == "IX")
```


## Plotting Scripts

Load in pre-processed data here:  

```{r}
#Load in data
Fluconazole_CSS1_glm <- readRDS("Fluconazole_CSS1_glm_J25.rds")
Fluconazole_CSS8_glm <- readRDS("Fluconazole_CSS8_glm_J25.rds")

#Load in peaks
peaks_8 <- readRDS("Fluc_CSS8_peaks_w700.rds")
peaks_1 <- readRDS("Fluc_CSS1_peaks_w700.rds")

rbind(peaks_1, peaks_8) -> FlucPeaks

#Load in cutoffs
Fluconazole_CSS8_q5 <- readRDS("Fluconazole_CSS8_q5_J25.rds")
Fluconazole_CSS1_q5 <- readRDS("Fluconazole_CSS1_q5_J25.rds")

Fluconazole_CSS8_q5 %>% mutate(CSS = "Fixed VIII") %>% filter(label %in% c("Bulk", "Interaction"))-> temp
...
rbind(Fluconazole_CSS1_glm, Fluconazole_CSS8_glm) %>% filter(label %in% c("Bulk", "Interaction")) -> DataToPlot

DataToPlot %>% mutate(CHROM = factor(CHROM, levels = as.character(as.roman(1:16)))) -> dfplot
...
Fluconazole_CSS1_glm %>% filter(CHROM == "VIII") %>% 
  ungroup() %>%
  summarize(min(POS), max(POS))

CHROM_VIII <- data.frame(CHROM = "VIII",
                         POS = c(0, 524417),
                         zscore = NA,
                         label = "Bulk",
                         Selection = NA,
...
Fluconazole_CSS8_glm %>% rbind(CHROM_VIII) %>%
...
mutate(CHROM = factor(CHROM, levels = as.character(as.roman(1:16)))) %>%
  ...
  geom_hline(data = temp, aes(yintercept = quant), linetype = "dashed") +
  #geom_point(data = FlucPeaks) + #this is being weird and inconsistent
  facet_grid( ~ CHROM, scales = "free", space = "free") +
  scale_color_manual(values = c("black", "orange")) +
  ylab("|zscore|") + xlab("") + 
  #ylim(0, 12) + 
  theme_bw() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), legend.position = "none") -> zoomout

Fluconazole_CSS8_glm %>% rbind(CHROM_VIII) %>%
  filter(CHROM %in% c("XVI", "VIII")) %>%
     filter(label %in% c("Bulk", "Interaction")) %>%
  #mutate(CHROM = factor(CHROM, levels = as.character(as.roman(1:16)))) %>%
  ggplot(aes(x = POS, y = abs(zscore), color = label)) + geom_line() +
  geom_hline(data = temp, aes(yintercept = quant), linetype = "dashed") +
  facet_grid( ~ CHROM, scales = "free", space = "free") +
  scale_color_manual(values = c("black", "orange")) +
  ylab("|zscore|") + xlab("Position") + 
  #ylim(0, 12) + 
  theme_bw() + 
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), legend.position = "bottom") -> zoomin
  
cowplot::plot_grid(zoomout, zoomin, ncol = 1, rel_heights = c(1,2))
