...
# Plotting Together
```{r}
BulkPeaks <- readRDS("BulkCutoffs_cf.rds") %>% filter(label != "BLANK")
IntPeaks <- readRDS("IntCutoffs_cf.rds") %>% filter(label != "BLANK")
...
saveRDS(alloutput_copper, file = "covfilt/bootstrap_Copper")
...
saveRDS(alloutput_Cycloheximide, file = "covfilt/bootstrap_Cycloheximide.rds")
...
saveRDS(alloutput_H2O2, file = "covfilt/bootstrap_H2O2.rds")
...
saveRDS(alloutput_Fluconazole, file = "covfilt/bootstrap_fluconazole.rds")
...
saveRDS(alloutput_Paraquat, file = "covfilt/bootstrap_paraquat.rds")
...
saveRDS(alloutput_Ethanol, file = "covfilt/bootstrap_ethanol.rds")
...
saveRDS(alloutput_Zeocin, file = "covfilt/bootstrap_zeocin.rds")
...
saveRDS(Chr1v8cor_copper, file = "covfilt/BS_Copper")
...
saveRDS(Chr1v8cor_Cycloheximide, file = "covfilt/BS_Cycloheximide.rds")
...
saveRDS(Chr1v8cor_H2O2, file = "covfilt/BS_H2O2.rds")
...
saveRDS(Chr1v8cor_Fluconazole, file = "covfilt/BS_fluconazole.rds")
...
saveRDS(Chr1v8cor_Paraquat, file = "covfilt/BS_paraquat.rds")
...
saveRDS(Chr1v8cor_Ethanol, file = "covfilt/BS_ethanol.rds")
...
saveRDS(Chr1v8cor_Zeocin, file = "covfilt/BS_zeocin.rds")
...
#saveRDS(All_Bootstraps, file = "covfilt/CorrBootstraps.rds")
...
```{r}
# Copper_G200 <- readRDS()
# Copper_Unsmoothed_cf <- readRDS()

Copper_G200 %>% filter(Dataset == "HKTFTDRX2_n01_OCa") %>%
  pivot_wider(names_from = Allele, values_from = SmoothCount) %>%
  unnest() %>%
  mutate(logOdds = log(Wine/Oak),
         Smooth = TRUE) -> sm_Cu

CuSO4_2024 %>% filter(Dataset == "HKTFTDRX2_n01_OCa") %>%
  pivot_wider(names_from = Allele, values_from = Reads) %>%
  unnest() %>%
  mutate(logOdds = log(Wine/Oak),
         Smooth = FALSE) -> unsm_Cu

sm_Cu %>% ggplot(aes(x = POS, y = logOdds)) +
  geom_point(data = unsm_Cu, color = "turquoise", alpha = 0.4) +
