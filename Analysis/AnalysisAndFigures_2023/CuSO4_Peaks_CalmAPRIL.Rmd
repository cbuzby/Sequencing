...
```{r, eval = FALSE}
contrasts(HTG3TDMXY_prep_highdose$Bulk) <- matrix(c(0, 1), ncol = 1)
contrasts(HTG3TDMXY_prep_highdose$Parent) <- matrix(c(1/2, -1/2), ncol = 1)

contrasts(HTG3TDMXY_prep_lowdose$Bulk) <- matrix(c(0, 1), ncol = 1)
contrasts(HTG3TDMXY_prep_lowdose$Parent) <- matrix(c(1/2, -1/2), ncol = 1)
...
saveRDS(HTG3TDMXY_glm_highdose, file = "HTG3TDMXY_glm_highdose_APRIL24.rds")
...
saveRDS(HTG3TDMXY_glm_lowdose, file = "HTG3TDMXY_glm_lowdose_APRIL24.rds")
```
...
saveRDS(HTG3TDMXY_permutation_Qs, file = "HTG3TDMXY_permutation_Qs_APRIL24.rds")
```
...
saveRDS(HVYTYDRX2_glm, file = "HVYTYDRX2_glm_APRIL24.rds")
...
saveRDS(HVYTYDRX2_permutation_Qs, file = "HVYTYDRX2_permutation_Qs_APRIL24.rds")
```
...
contrasts(HKTFTDRX2_prep$Bulk) <- matrix(c(0, 1), ncol = 1)
...
saveRDS(HKTFTDRX2_glm, file = "HKTFTDRX2_glm_APRIL24.rds")
...
saveRDS(HKTFTDRX2_permutation_Qs, file = "HKTFTDRX2_permutation_Qs_APRIL24.rds")
```
...
contrasts(HNGLVDRXY_prep$Bulk) <- matrix(c(0, 1), ncol = 1)
...
saveRDS(HNGLVDRXY_glm, file = "HNGLVDRXY_glm_APRIL24.rds")
...
saveRDS(HNGLVDRXY_permutation_Qs, file = "HNGLVDRXY_permutation_Qs_APRIL24.rds")
```
...
```{r}
HTG3TDMXY_permutation_Qs <- readRDS("HTG3TDMXY_permutation_Qs_APRIL24.rds")
HNGLVDRXY_permutation_Qs <- readRDS("HNGLVDRXY_permutation_Qs_APRIL24.rds")
HKTFTDRX2_permutation_Qs <- readRDS("HKTFTDRX2_permutation_Qs_APRIL24.rds")
HVYTYDRX2_permutation_Qs <- readRDS("HVYTYDRX2_permutation_Qs_APRIL24.rds")

```

```{r}

HTG3TDMXY_glm_lowdose <- readRDS("HTG3TDMXY_glm_lowdose_APRIL24.rds")
HTG3TDMXY_glm_highdose <- readRDS("HTG3TDMXY_glm_highdose_APRIL24.rds")

HNGLVDRXY_glm <- readRDS("HNGLVDRXY_glm_APRIL24.rds") 
HKTFTDRX2_glm <- readRDS("HKTFTDRX2_glm_APRIL24.rds")
HVYTYDRX2_glm <- readRDS("HVYTYDRX2_glm_APRIL24.rds")
...
CuSO4_All %>% 
  ...
  geom_line(aes(linetype = paste(Pool, Dose)), color = "black") +
...
```{r, fig.width = 12, fig.height = 5}
#Where are the chaperones that have something to do with copper?
sgd_orfs %>% filter(Gene %in% c("PEP8", "CCH1","PEP1", "SYS1","VPS35","SQT1", "VPS5")) -> CopperChaperones

Cugenes <- c("CRS5", "SOD1", "CUP1-1", "CUP1-2", "MET17", "MUP1", "PCL1", "OYE3", "FRE1", "FRE7", "CTR1", "IRC7")
sgd_orfs %>% filter(Gene %in% Cugenes) -> CopperGenes #there aren't any?
#myloci

Cugenes_eh <- c("VMA4", "MTL1", "PTR2", "LEU9", "IRC15")
sgd_orfs %>% filter(Gene %in% Cugenes_eh) -> sgd_copper_eh

CuSO4_All %>% 
  ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + 
  geom_hline(aes(yintercept = quant095, color = Factor)) + 
  
  geom_vline(data = CopperChaperones, aes(xintercept = Start), alpha = 0.4, size = 1, color = "turquoise") +
  geom_vline(data = CopperGenes, aes(xintercept = Start), alpha = 0.4, size = 1, color = "darkred") +
  geom_vline(data = myloci[myloci$cat == "X",], aes(xintercept = POS), alpha = 0.4, size = 1, color = "red") +
  geom_vline(data = myloci[myloci$cat == "Floc",], aes(xintercept = POS), alpha = 0.4, size = 1, color = "orange") +
...
#LIST OUT WHO IS ON WHICH
peakgenes <- data.frame(Gene = c("CUP1-1", "CUP1-2", "FRE1", "URA3", "VPS5", "CTR1", "PCL1"),
                        Cat = c("Cu", "Cu", "Cu", "X", "chap", "Cu", "Cu"))

sgd_orfs %>% merge(peakgenes) -> sgd_annotate

unknownpeaks <- data.frame(CHROM = c("II", "IV", "IX", "VII", "XI", "XIV", "XVI"),
                           POS.Start.Mult = c(0.5, 0.7, 0.2, 0.3, 0, 0.45, 0.25),
                           POS.End.Mult = c(0.7, 1, 0.8, 0.65, 1, 0.75, 0.8))

ChromosomeScale %>% filter(POS != 1) %>% select(CHROM, POS) %>% merge(unknownpeaks) %>% mutate(Start = POS.Start.Mult*POS, End = POS.End.Mult*POS) %>% select(CHROM, Start, End) %>% mutate(Factor = "Bulk") -> unknownpeaksannotated

CuSO4_All %>% 
  ggplot() + 
  geom_rect(data = unknownpeaksannotated,
            aes(xmin = Start, xmax = End, ymin = -Inf, ymax = Inf),
            alpha = 0.1, fill = "orange") +
  geom_vline(data = sgd_annotate, aes(xintercept = Start, color = Cat), alpha = 0.4, size = 1) +
  
  geom_line(aes(x = POS, y = abs(Summary), linetype = paste(Pool, Dose)), alpha = 0.8, show.legend = FALSE) +
  facet_grid(Factor~CHROM, space = "free", scales = "free") + ggtitle("Comparison of Runs") +

  theme(legend.position = "bottom")

#Doing this but for percentage above cutoff
CuSO4_All %>% mutate(Score = log(abs(Summary)/quant095)) %>% filter(Score >= 0) %>%
  ggplot() + 
  geom_rect(data = unknownpeaksannotated,
            aes(xmin = Start, xmax = End, ymin = -Inf, ymax = Inf),
            alpha = 0.1, fill = "orange") +
  ...
  geom_line(aes(x = POS, y = Score, linetype = paste(Pool, Dose)), alpha = 0.8, show.legend = FALSE) +
  facet_grid(Factor~CHROM, space = "free", scales = "free") + ggtitle("Comparison of Runs") +

  theme(legend.position = "bottom")

CuSO4_All %>% mutate(Score = log(abs(Summary)/quant095)) %>% 
  filter(Score >= 0) %>%
  select(Factor, CHROM, Dose, POS, Score, Pool) %>%
  pivot_wider(names_from = c(Pool,Dose), values_from = Score) %>%
  na.omit() %>%
  pivot_longer(c(-Factor, -CHROM, -POS), names_to = c("Pool", "Dose"), values_to = "Score", names_sep = "_") %>%
  ggplot() + 
  geom_line(data = CuSO4_All, aes(x = POS, y = abs(Summary), linetype = paste(Pool, Dose)), alpha = 0.8, show.legend = FALSE) +
  geom_vline(aes(xintercept = POS), size = 0.01, alpha = 0.01, color = "orchid") +
  facet_grid(Factor~CHROM, space = "free", scales = "free") + 
  ggtitle("Significant in all CuSO4 Experiments")
```

Okay so if we find the peak of the regions that are highlighted, this might give us a good idea of where the max value or gene actually is
...
CuSO4_All %>% mutate(Score = log(abs(Summary)/quant095)) %>% 
  ...
  pivot_wider(names_from = c(Pool,Dose), values_from = Score) %>%
  na.omit() %>%
  pivot_longer(c(-Factor, -CHROM, -POS), names_to = c("Pool", "Dose"), values_to = "Score", names_sep = "_") -> AllSampleBulks

AllSampleBulks %>% group_by(CHROM, Factor) %>% 
  summarize(end = max(POS), start = min(POS)) %>% 
  rbind(data.frame(CHROM = "XII", Factor = "Bulk", end = 400000, start = 320335)) %>%
  merge(CuSO4_All) %>% 
  filter(POS > start & POS < end) %>%
  ungroup() %>%
  group_by(Pool, CHROM) %>% summarize(Summary = max(abs(Summary))) %>% merge(CuSO4_All) -> TruePeaks

TruePeaks %>% group_by(CHROM) %>% summarize(true = mean(POS)) -> TruePeak_Avg
...
CuSO4_All %>% filter(Factor == "Bulk") %>%
  ...
  geom_rect(data = unknownpeaksannotated,
            aes(xmin = Start, xmax = End, ymin = -Inf, ymax = Inf),
            alpha = 0.1, fill = "orange") +
  geom_vline(data = TruePeak_Avg, aes(xintercept = true), alpha = 0.4, size = 1, color = "orchid") +
...
sgd_orfs %>% ungroup() %>% select(-POS.Start, -POS.End) %>% 
  filter(CHROM %in% TruePeak_Avg$CHROM) %>% merge(TruePeak_Avg) %>% group_by(CHROM) %>% 
  filter(true < (End + 1000), true > (Start - 1000)) -> potentialgenes
```
...
```{r, fig.width=12, fig.height=4}
#Separating by CSS only
CuSO4_All_doses %>% mutate(Prop95 = log(abs(Summary)/quant095)) %>%
  filter(Factor == "Bulk") %>%
  ggplot(aes(x = POS, y = Prop95, linetype = paste(Dose, Pool))) + geom_line() + 
  geom_vline(data = addtoplot2[addtoplot2$num == 1  & addtoplot2$Factor == "Bulk",], 
             aes(xintercept = avgPOS), color = "gray", 
             alpha = 0.4, size = 1) +
  geom_vline(data = addtoplot2[addtoplot2$num > 1  & addtoplot2$Factor == "Bulk",], 
...
```{r, fig.width=12, fig.height=4}
#Separating by Dose only
CuSO4_All_doses %>% mutate(Prop95 = log(abs(Summary)/quant095)) %>%
  filter(Factor == "Bulk") %>%
  ggplot(aes(x = POS, y = Prop95, linetype = paste(CSS, Pool))) + geom_line() + 
  geom_vline(data = addtoplot2[addtoplot2$num == 1  & addtoplot2$Factor == "Bulk",], 
             aes(xintercept = avgPOS), color = "gray", 
             alpha = 0.4, size = 1) +
  geom_vline(data = addtoplot2[addtoplot2$num > 1  & addtoplot2$Factor == "Bulk",], 
             ...
             alpha = 0.4, size = 1) +
  ylim(0, 4) +
  facet_grid(Dose~CHROM, space = "free", scales = "free") +
  theme(legend.position = "none") + ggtitle("Proportion of 95% Cutoff | Bulk")
