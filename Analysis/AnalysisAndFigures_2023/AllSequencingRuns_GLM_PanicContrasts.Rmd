...
```{r}
rbind(HTG3TDMXY_CSS8_contrastGLM,
      HVYTYDRX2_CuSO4_CSS8_contrastGLM,
      HKTFTDRX2_CSS1_contrastGLM,
      HNGLVDRXY_CSS1_contrastGLM) -> CuSO4_withcontrasts

CuSO4_withcontrasts %>% ggplot(aes(x = POS, y = Effect, color = labels)) + geom_line() + facet_grid(Pool~CHROM, scales = "free", space = "free")
```
...
testdata_c2 %>% group_by(Pool, Index) %>%
  summarize(effect = glm_cb2_effect(Allele = Allele, Bulk = Bulk,  
                           Bulk = Bulk,
                           Parent = Parent,
                           W = count,
