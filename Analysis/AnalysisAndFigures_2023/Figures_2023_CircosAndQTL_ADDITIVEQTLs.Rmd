...
cybr_multpeaks <- function(dataset, 
  ...
  for(i in 100:nrow(dataset)){
    if(dataset$Window[i] == dataset$Window[(i-9)]){
      dataset$Check[(i-90):i] <- FALSE
    }else{
      dataset$Check[(i-90):i] <- TRUE
    }
  }
...
cybr_multpeaks
F_1_intpeaks <- cybr_callpeaks(Fluc_glm, param = "Bulk")
C_1_intpeaks <- cybr_callpeaks(CuSO4_glm, param = "Bulk")
H_8_intpeaks <- cybr_callpeaks(H2O2_8_glmer, param = "Bulk")
Z_1_intpeaks <- cybr_callpeaks(Zeocin_glm, param = "Bulk")
C_8_intpeaks <- cybr_callpeaks(CuSO4_8_glm, param = "Bulk")
F_8_intpeaks <- cybr_callpeaks(Fluc_8_glm, param = "Bulk")
Z_8_intpeaks <- cybr_callpeaks(Zeocin_8_glmer, param = "Bulk")
...
#GLMER PEAKS
H8g_intpeaks <- cybr_callpeaks(H2O2_8_glmer, param = "Bulk")
C1g_intpeaks <- cybr_callpeaks(CuSO4_1_glmer, param = "Bulk")
H1g_intpeaks <- cybr_callpeaks(H2O2_glmer, param = "Bulk")
Z8g_intpeaks <- cybr_callpeaks(Zeocin_8_glmer, param = "Bulk")
F1g_intpeaks <- cybr_callpeaks(Fluc_1_glmer, param = "Bulk")
...
c1p <- cybr_callpeaks(CuSO4_glm, param = "Bulk", threshold = 0.4098215, include_all = TRUE)
c8p <- cybr_callpeaks(CuSO4_8_glm, param = "Bulk", threshold = 0.4839316, include_all = TRUE)
f1p <- cybr_callpeaks(Fluc_glm, param = "Bulk", threshold = 0.4260583, include_all = TRUE)
...
```{r, fig.width=16, fig.height=4}
c1pn <- cybr_callpeaks(CuSO4_glm, param = "Bulk", threshold = 0.4098215)
c8pn <- cybr_callpeaks(CuSO4_8_glm, param = "Bulk", threshold = 0.4839316)
f1pn <- cybr_callpeaks(Fluc_glm, param = "Bulk", threshold = 0.4260583)
z1pn <- cybr_callpeaks(Zeocin_glm, param = "Bulk", threshold = 0.4024422)
h8pn <- cybr_callpeaks(H2O2_8_glmer, param = "Bulk", threshold = 2.0160408)
z8pn <- cybr_callpeaks(Zeocin_8_glmer, param = "Bulk", threshold = 2.0160408)
f8pn <- cybr_callpeaks(Fluc_8_glm, param = "Bulk", threshold = 0.4839316)
...
```{r, fig.width=16, fig.height=4}
mywidth <- 400000
F_1_intpeaks_m <- cybr_multpeaks(Fluc_glm, param = "Bulk", width = mywidth)

Fluc_glm %>% filter(CHROM == "II") %>%
      ggplot() +
  geom_rect(aes(xmin = 0, xmax = mywidth, ymin = 0, ymax = Inf), fill = "lightblue", alpha = 0.2) +
      geom_vline(data = F_1_intpeaks_m[F_1_intpeaks_m$CHROM == "II",], 
...
c1pn_Genes <- cybr2_SGDGenes(peaks = c1pn, mywindow = 6000) 
f1pn_Genes <- cybr2_SGDGenes(peaks = f1pn, mywindow = 4000)
z1pn_Genes <- cybr2_SGDGenes(peaks = z1pn, mywindow = 4000)

c8pn_Genes <- cybr2_SGDGenes(peaks = c8pn, mywindow = 6000) 
...
#Chr 7
C_glm %>% filter(label == "Bulk", CHROM == c("VII")) %>% ggplot(aes(x = POS, y =  (summary), linetype = Fixed)) +
  geom_line() +
  geom_vline(data = c_Genes[c_Genes$CHROM == "VII",], aes(xintercept = POS_Start, color = StandardName, linetype = Fixed)) +
  facet_grid(~CHROM, scales = "free", space = "free") 

c_Genes[c_Genes$CHROM == "VII",] %>% arrange(POS_Start)

#Chr 12
C_glm %>% filter(label == "Bulk", CHROM == c("XII")) %>% ggplot(aes(x = POS, y =  (summary), linetype = Fixed)) +
  geom_line() +
...
#Chromosome XV
C_glm %>% filter(label == "Bulk", CHROM == c("XV")) %>% ggplot(aes(x = POS, y =  (summary), linetype = Fixed)) +
  geom_line() +
...
#Chromosome XVI
C_glm %>% filter(label == "Bulk", CHROM == c("XVI")) %>% ggplot(aes(x = POS, y =  (summary), linetype = Fixed)) +
  geom_line() +
