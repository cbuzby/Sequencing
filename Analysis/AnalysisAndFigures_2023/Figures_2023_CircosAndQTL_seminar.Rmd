...
Areas of definite non-heterozygosity
...
F_1_intpeaks <- cybr_callpeaks(Fluc_1_glmer, param = "Interaction")
C_1_intpeaks <- cybr_callpeaks(CuSO4_1_glmer, param = "Interaction")
...
CSSCutoff_new %>% filter(labels == "Interaction")

#USE FOR GLMER ONES
C_1_intpeaks_g <- cybr_callpeaks(CuSO4_1_glmer, param = "Interaction", threshold = 2.808175)
C_8_intpeaks_g <- cybr_callpeaks(CuSO4_8_glm, param = "Interaction", threshold = 8.691379)

F_1_intpeaks_g <- cybr_callpeaks(Fluc_1_glmer, param = "Interaction", threshold = 1.872063)
#F_8_intpeaks_g <- cybr_callpeaks(Fluc_8_glm, param = "Interaction", threshold = 1.96)

#H_1_intpeaks_g <- cybr_callpeaks(H2O2_glmer, param = "Interaction", threshold = 0.3141185)

H_8_intpeaks_g <- cybr_callpeaks(H2O2_8_glmer, param = "Interaction", threshold = 7.168438)

Z_8_intpeaks_g <- cybr_callpeaks(Zeocin_8_glmer, param = "Interaction", threshold = 7.168438)
Z_1_intpeaks_g <- cybr_callpeaks(Zeocin_glm, param = "Interaction", threshold = 5.626115)
...
Fluc_glm %>% rbind(ChromosomeScale2) %>%
  ...
  geom_line(size = 1.2) + 
    ...
    ylab("") + xlab("")+ ggtitle("")+
  facet_grid(~CHROM, scales = "free", space = "free")  + 
  scale_color_manual(values = c("black",  "#7030A0", "gray")) + 
  #scale_color_manual(values = c("gray",  "black","#7030A0")) + 
  theme(legend.position = "none", axis.text.x=element_blank(),
...
H2O2_8_glmer %>% select(CHROM = CHROM, POS = POS, Summary = summary, Label = label) %>% rbind(ChromosomeScale) %>%
  ...
  geom_line(size = 1.2) + 
    geom_hline(aes(yintercept = 7.168438), linetype = "dashed", color = "#7030A0") +
    geom_hline(aes(yintercept = 6.385656), linetype = "dashed", color = "black") +
    ylab("") + xlab("")+ ggtitle("")+
  facet_grid(~CHROM, scales = "free", space = "free")  + 
  scale_color_manual(values = c("gray",  "black","#7030A0")) + 
  theme(legend.position = "none", axis.text.x=element_blank(),
...
H2O2_glmer %>% select(CHROM = CHROM, POS = POS, Summary = summary, Label = label) %>% rbind(ChromosomeScale) %>%
  ggplot(aes(x = POS, y = abs(Summary), color = Label)) + 
    geom_vline(data = H_1_intpeaks_g, aes(xintercept = POS), color = "#7DB0B0", size = 2) +
  geom_line(size = 1.2) + 
...
cybrPurple <- function(dataset, cutoff = "III", peakslist = NULL, includeFixedGenes = TRUE, peakcolor = "#7DB0B0", mylim = NULL){
  ...
  dataset$CHROM <- factor(dataset$CHROM, levels = as.character(as.roman(1:16)))
  dataset$label <- factor(dataset$label, levels = c("Bulk", "Parent", "Interaction", "Rep", "Intercept"))
    
  dataset %>% select(CHROM = CHROM, POS = POS, Summary = summary, Label = label) %>% rbind(ChromosomeScale) %>%
    ggplot(aes(x = POS, y = abs(Summary), color = Label, linetype = Label == "Parent")) -> BasePlot
  
  #Add fixed genes to plot
  if(includeFixedGenes == TRUE){
    BasePlot <- BasePlot + geom_vline(data = FG, aes(xintercept = POS), color = "gray", size = 2)
  }
...
cybrPurple(H2O2_8_glmer, mylim = 8, peakslist = H_8_intpeaks)

cybrPurple(CuSO4_8_glm, mylim = 8, peakslist = C_8_intpeaks)

#dataset <- H2O2_8_glmer
```
...
rbind(C8_14, C1_14, H8_14, H1_14, Z8_14, Z1_14, F8_14, F1_14) %>%
  ...
  geom_vline(xintercept = 467131) #MKT1 375215 #FLO8 on V
...
```{r}
CuSO4_8_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(Exp = "C8") -> C8_5
CuSO4_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(Exp = "C1") -> C1_5
H2O2_8_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(summary = Summary, label = Label, Exp = "H8") -> H8_5
H2O2_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(summary = Summary, label = Label, Exp = "H1") -> H1_5
Zeocin_8_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(summary = Summary, label = Label, Exp = "Z8") -> Z8_5
Zeocin_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(Exp = "Z1") -> Z1_5
Fluc_8_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(Exp = "F8") -> F8_5
Fluc_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "V") %>% mutate(Exp = "F1") -> F1_5

URA3 <- data.frame(Gene = "Ura3",
                   CHROM = "V",
                   POS = 116167)

rbind(C8_5, C1_5, H8_5, H1_5, Z8_5, Z1_5, F8_5, F1_5) %>%
  filter(label == "Bulk") %>%
  ggplot(aes(x = POS, y = abs(summary), color = Exp)) + geom_point(alpha = 0.5) +
  ggtitle("Bulk Effects, Chr 5")+
  geom_vline(xintercept = 116167, linetype = "dashed") +
  geom_vline(xintercept = 375215) #FLO8

rbind(C8_5, C1_5, H8_5, H1_5, Z8_5, Z1_5, F8_5, F1_5) %>%
  filter(label == "Interaction") %>%
  ggplot(aes(x = POS, y = abs(summary), color = Exp)) + geom_point(size = 1, alpha = 0.4)+
  ggtitle("Interaction Effects, Chr 5") +
  xlim(0, max(C8_5$POS)) +
    geom_vline(xintercept = 116167, linetype = "dashed") +
  geom_vline(xintercept = 375215) #FLO8 - rule out flocculation
  
```

```{r}
CuSO4_8_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(Exp = "C8") -> C8_9
CuSO4_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(Exp = "C1") -> C1_9
H2O2_8_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(summary = Summary, label = Label, Exp = "H8") -> H8_9
H2O2_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(summary = Summary, label = Label, Exp = "H1") -> H1_9
Zeocin_8_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(summary = Summary, label = Label, Exp = "Z8") -> Z8_9
Zeocin_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(Exp = "Z1") -> Z1_9
Fluc_8_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(Exp = "F8") -> F8_9
Fluc_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "IX") %>% mutate(Exp = "F1") -> F1_9

rbind(C8_9, C1_9, H8_9, H1_9, Z8_9, Z1_9, F8_9, F1_9) %>%
  filter(label == "Bulk") %>%
  ggplot(aes(x = POS, y = abs(summary), color = Exp)) + geom_point(alpha = 0.5) +
  ggtitle("Bulk Effects, Chr 9")+
  geom_vline(xintercept = 389572) #FLO11

rbind(C8_9, C1_9, H8_9, H1_9, Z8_9, Z1_9, F8_9, F1_9) %>%
  filter(label == "Interaction") %>%
  ggplot(aes(x = POS, y = abs(summary), color = Exp)) + geom_point(size = 1, alpha = 0.4)+
  ggtitle("Interaction Effects, Chr 9") +
    xlim(0,439888) +
  geom_vline(xintercept = 389572) #FLO11 - rule out flocculation
```

```{r}
CuSO4_8_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(Exp = "C8") -> C8_3
CuSO4_1_glmer %>% rbind(ChromosomeScale2) %>% filter(CHROM == "III") %>% mutate(Exp = "C1") -> C1_3
H2O2_8_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(summary = Summary, label = Label, Exp = "H8") -> H8_3
H2O2_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(summary = Summary, label = Label, Exp = "H1") -> H1_3
Zeocin_8_glmer %>% mutate(Summary = summary, Label = label) %>% select(-summary, -label) %>%
  rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(summary = Summary, label = Label, Exp = "Z8") -> Z8_3
Zeocin_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(Exp = "Z1") -> Z1_3
Fluc_8_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(Exp = "F8") -> F8_3
Fluc_glm %>% rbind(ChromosomeScale) %>% filter(CHROM == "III") %>% mutate(Exp = "F1") -> F1_3

rbind(C8_3, C1_3, H8_3, H1_3, Z8_3, Z1_3, F8_3, F1_3) %>%
  filter(label == "Bulk") %>%
  ggplot(aes(x = POS, y = abs(summary), color = Exp)) + geom_point(alpha = 0.5) +
  ggtitle("Bulk Effects, Chr III")+
    xlim(0,max(C8_3$POS)) +
  geom_vline(xintercept = c(198671,201177)) + #MATalpha
  geom_vline(xintercept = c(114385,114501), color = "red") #CEN3

rbind(C8_3, C1_3, H8_3, H1_3, Z8_3, Z1_3, F8_3, F1_3) %>%
  filter(label == "Interaction") %>%
  ggplot(aes(x = POS, y = abs(summary), color = Exp)) + geom_line(size = 1)+
  ggtitle("Interaction Effects, Chr III") +
  xlim(0,max(C8_3$POS)) +
