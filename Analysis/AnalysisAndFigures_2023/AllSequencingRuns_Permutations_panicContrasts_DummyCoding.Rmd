...
rbind(HTG3TDMXY_glm_Ec[HTG3TDMXY_glm_Ec$CHROM != "VIII",], 
      #HTG3TDMXY_glm_Ec01_rep[HTG3TDMXY_glm_Ec01_rep$CHROM != "VIII",],
...
IDkey <- data.frame(Pool = c("HTG3TDMXY", "HNGLVDRXY", "HKTFTDRX2", "HVYTYDRX2"),
                    CSS = c("VIII", "I", "I", "VIII"),
                    Label = c("CSS8 B", "CSS1 A", "CSS1 B", "CSS8 A"),
                    Selection = c("High", "High", "Low", "Low"))

CuSO4_contrastglm_cutoffs %>% merge(IDkey) -> CuSO4_contrastglm_cutoffs
...
CuSO4_contrastglm_cutoffs %>% group_by(Pool, Label, Factor, CHROM, d) %>% arrange(POS) %>%
  na.omit() %>%
  ...
  group_by(Pool, Label, Factor, CHROM, d, segment_id) %>%
...
CuSO4_contrastglm_cutoffs %>% filter(d == "Z", Factor != "cept", Factor != "Parent") %>% ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + 
  ...
  geom_point(size = 0.4, alpha = 0.5) + facet_grid(Label ~ CHROM, space = "free", scales = "free") +
  ggtitle("Z scores, 0/1, Interaction Peaks") +
...
```{r}
CuSO4_contrastglm_cutoffs %>% filter(d == "Z", Factor == "Bulk", CHROM == "XI") %>% ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + 
  geom_vline(data = maybepeaks[maybepeaks$Factor == "Bulk" & 
                                 maybepeaks$d == "Z" & 
                                 maybepeaks$CHROM == "XI",], 
             aes(xintercept = ppeakPOS), color = "black", linewidth = 2, alpha = 0.3) +
  geom_hline(aes(yintercept = quant095)) + 
  geom_line() + facet_grid(Label ~ CHROM, space = "free", scales = "free") +
  ggtitle("Z scores, 0/1, Peaks")
```