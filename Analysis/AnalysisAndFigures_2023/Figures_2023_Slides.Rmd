...
ggplot2::theme_set(theme_light() + theme(text = element_text(size = 20)) + theme(legend.position = "bottom", #axis.text.x=element_blank(),
...
## Smooth Data in a different way

Not evaluated because I compare things from further down, but can move down to look at plots if needed

```{r, eval = FALSE}
windowsize = 50
CSSI_CuSO4 %>% pivot_longer(c(-CHROM, -POS), names_to = "label") %>% group_by(CHROM, label) %>% arrange(POS) %>%
  summarize(POS = POS, CHROM = CHROM, SmoothCount = ceiling(frollapply(value, n = windowsize, FUN = sum, align = "left"))) %>% na.omit() %>% pivot_wider(names_from = label,values_from = SmoothCount) -> CSSI_CuSO4_rollSum

mysubset <- CSSI_CuSO4_rollSum[seq(nrow(CSSI_CuSO4_rollSum)) %% windowsize == 0, ]


mysubset %>% pivot_longer(cols = c(-CHROM, -POS),
  names_to = c("Bulk", "Parent", "Allele"),
    names_sep = "_",
  values_to = "Reads") %>%
  pivot_wider(names_from = Allele, values_from = Reads) %>%
  mutate(LOD = log(Wine/Oak)) -> CSSI_CuSO4_rollSum_LOD

CSSI_CuSO4_rollSum_LOD %>% filter(CHROM != "I", CHROM != "M") %>% ggplot(aes(x = POS, y = LOD, color = paste(Bulk, Parent))) + geom_point(alpha = 0.4) + 
  facet_grid(~CHROM, scales = "free", space = "free") + 
  theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank())

mysubset %>% group_by(CHROM, POS) %>% 
  summarise(summary = glmfixed(HOO = CuSO4_Oak_Oak, 
                           ...
                           LWW = Dilute_Wine_Wine)[1:4],
                                                   label = c("intercept", "Bulk", "Parent", "Interaction")) -> GLMdata_sums

GLMdata_sums$label <- factor(GLMdata_sums$label, levels = c("Rep", "Parent", "Bulk", "Interaction"))

GLMdata_sums %>% filter(label != "intercept", CHROM != "I") %>% ggplot(aes(x = POS, y = summary, color = label)) + geom_line() + facet_grid(~CHROM, scales = "free", space = "free") + 
  theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) + ggtitle(paste("Smooth by Adding | Window ", windowsize))

CuSO4_glm %>% filter(label != "intercept", CHROM != "I") %>% ggplot(aes(x = POS, y = summary, color = label)) + geom_line() + facet_grid(~CHROM, scales = "free", space = "free") + 
  theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) + ggtitle("Original Smoothing")

getwd()

```

Plot to compare (this hasn't been run yet if knitting so don't eval)

```{r, eval = FALSE, fig.width=16, fig.height=6}
CSSI_CuSO4_LOD %>% filter(CHROM != "I", CHROM != "M") -> test

CSSI_CuSO4_rollSum_LOD %>% filter(CHROM != "I", CHROM != "M") %>% ggplot(aes(x = POS, y = LOD, color = paste(Bulk, Parent))) + 
...
#Comparison of just one chromosome
AllLODs_unsmoothed %>% filter(CHROM == "VII", Dataset == "Fluconazole") %>% 
  ggplot() + 
  geom_point(aes(x = POS, y = LOD, color = paste(Bulk, Parent), shape = Rep), size = 0.4, alpha = 0.4)+
  geom_line(data = AllLODs[AllLODs$Dataset == "Fluconazole" & AllLODs$CHROM == "VII",], mapping = aes(x = POS, y = LOD, group = Rep), color = "black", size = 1)+
  facet_grid(~paste(Bulk, Parent), scales = "free", space = "free") + 
  scale_color_manual(values = c("gray31","gray","#5391C4", "firebrick", "gray")) + 
  theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) + 
  ylim(-2,2)+
  ggtitle("")

AllLODs_unsmoothed %>% filter(CHROM == "VII", Dataset != "CuSO4_2") %>% 
  ...
  scale_color_manual(values = c("#345F6F","black","orchid", "firebrick")) + 
  theme(legend.position = "none", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) +
  ggtitle("Chr 7 Raw")

# AllLODs %>% filter(CHROM == "VII", Dataset != "CuSO4_2") %>% 
#   filter(Bulk != "NA") %>%
#   mutate(Flask = paste(Bulk, Parent, Rep, Dataset, sep = "_")) %>% 
#   ggplot(aes(x = POS, y = LOD, color = Bulk, group = Flask, linetype = Parent)) + 
#   geom_point(alpha = 0.2, size = 0.8) + ylim(-2.5,2.5)+
#       ylab("") + xlab("")+
#   facet_grid(~Dataset, scales = "free", space = "free") + 
#   scale_color_manual(values = c("#345F6F","black","orchid", "firebrick", "gray")) + 
#   theme(legend.position = "none", axis.text.x=element_blank(),
#             axis.ticks.x=element_blank()) +
#   ggtitle("Chf 7 Smoothed")
# 
# AllLODs %>% filter(CHROM == "VIII", Dataset != "CuSO4_2") %>% 
#   filter(Bulk != "NA") %>%
#   mutate(Flask = paste(Bulk, Parent, Rep, Dataset, sep = "_")) %>% 
#   ggplot(aes(x = POS, y = LOD, color = Bulk, group = Flask, linetype = Parent)) + 
#   geom_point(alpha = 0.2, size = 0.8) + #ylim(-2.5,2.5)+
...
#   theme(legend.position = "none", axis.text.x=element_blank(),
#             axis.ticks.x=element_blank()) +
#   ggtitle("Chr 8 Smoothed")


AllLODs_unsmoothed %>% filter(CHROM == "VII") %>% 
  mutate(Flask = paste(Bulk, Parent, Rep, Dataset, sep = "_")) %>% 
  filter(Dataset == "Fluconazole") %>%
  ggplot(aes(x = POS, y = LOD, color = paste(Bulk, Parent), group = Flask)) + 
  ylab("") + xlab("")+ geom_point(size = 0.5) + 
  geom_hline(aes(yintercept = 0), linetype = "dashed") +
  scale_color_manual(values = c("gray31","gray","#5391C4", "firebrick")) + 
  theme(legend.position = "bottom", axis.text.x=element_blank(), axis.text.y=element_blank(),
            axis.ticks.x=element_blank()) + 
  scale_x_continuous(expand = expansion(0)) + 
  scale_y_continuous(expand = expansion(0)) + 
  ylim(-0.8,0.8)+
  ggtitle("") -> chr7_unsmoothed_cd
...
AllLODs %>% filter(CHROM == "VII") %>% 
  ...
  filter(Dataset == "Fluconazole") %>%
  ggplot(aes(x = POS, y = LOD, color = paste(Bulk, Parent), group = Flask)) + 
  ylab("") + xlab("")+geom_line(size = 1) + geom_hline(aes(yintercept = 0), linetype = "dashed") +
  scale_color_manual(values = c("gray31","gray","#5391C4", "firebrick")) + 
  theme(legend.position = "bottom", axis.text.x=element_blank(), axis.text.y=element_blank(),
            axis.ticks.x=element_blank()) +
  ylim(-0.8,0.8)+
...
Fluc_glm %>% filter(CHROM == "VII", label != "intercept") %>%
  ggplot(aes(x = POS, y = -1*(summary), color = label)) + geom_line(size = 1) + 
  ylab("") + xlab("")+geom_hline(aes(yintercept = 0), linetype = "dashed") +
  scale_color_manual(values = c("black","#7030A0","gray", "red")) + 
  theme(legend.position = "bottom", axis.text.x=element_blank(), axis.text.y=element_blank(),
            axis.ticks.x=element_blank()) +
  ylim(-4,4)+
  ggtitle("") -> chr7_glm

plot_grid(chr7_unsmoothed_cd, chr7_LOD_cd, chr7_glm, ncol = 3)
```

```{r, fig.width=16,fig.height=5}

AllLODs %>%  mutate(Parent = replace(Parent, Parent == "Oak", "OakI"),
         Parent = replace(Parent, Parent == "Wine", "WineI")) %>%
