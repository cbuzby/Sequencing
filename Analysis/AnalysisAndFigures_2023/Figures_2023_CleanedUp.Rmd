...
CuSO4_8_glm %>% rbind(ChromosomeScale) %>%
  ...
  geom_line(size = 0.8) + 
    ...
    ylab("") + xlab("")+ ggtitle("CuSO4 CSS8 with Replicates")+
  facet_grid(~CHROM, scales = "free", space = "free")  + 
  scale_color_manual(values = c("red","gray",  "black","#345F6F")) + 
  theme(legend.position = "bottom", axis.text.x=element_blank(),
...
Fluc_8_glm %>% rbind(ChromosomeScale) %>%
  ...
  geom_line(size = 0.8) + 
    ...
    ylab("") + xlab("")+ ggtitle("Fluconazole CSS8 with Replicates")+
  facet_grid(~CHROM, scales = "free", space = "free")  + 
  scale_color_manual(values = c("red","gray",  "black","#345F6F")) + 
  theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) + ylim(0,10)
...
Flucb %>% ggplot(aes(x = POS, y = abs(summary), color = label)) + geom_line(size = 1.2) + 
    geom_hline(aes(yintercept = max(abs(CUSO4b$summary[CUSO4b$CHROM == "III" & CUSO4b$Exp == "CSS8"]))), linetype = "dashed") +
    ylab("") + xlab("")+ ggtitle("Fluconazole Comparison")+
  facet_grid(Exp ~ CHROM, scales = "free", space = "free")  + 
...
```{r, fig.width=17,fig.height=8}
Zeocin_CSS8 <- readRDS("C:/Users/cassa/OneDrive/Documents/GitHub/Sequencing/Analysis/AnalysisAndFigures_2023/Zeocin_CSS8_glm.rds")
Zeocin_CSS8 %>% filter(coefficient != "(Intercept)", CHROM != "VIII") %>% mutate(coefficient = str_replace(coefficient, "BulkZeocin", "Bulk"),
  ...
  filter(label == "Zscore") %>% transmute(CHROM = CHROM, POS = POS, summary = GLMResult, label = coefficient) -> Z8

rbind(data.frame(Z8, Exp = "CSS8"),
      data.frame(Zeocin_glm, Exp = "CSS1")) -> ZeoAll

ZeoAll %>% filter(CHROM != "M") %>% ggplot(aes(x = POS, y = abs(summary), color = label)) + geom_line(linewidth = 1, alpha = 0.8) +
  facet_grid(Exp~CHROM, scales = "free", space = "free") +
  theme(legend.position = "bottom", axis.text.x=element_blank(),
            axis.ticks.x=element_blank()) +
