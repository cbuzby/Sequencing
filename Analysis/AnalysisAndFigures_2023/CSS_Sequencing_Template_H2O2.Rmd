...
FilteredData_UNFILTERED <- cybrInputGATKTable(mydatatotest) %>% 
  cybrQualityFilter(GQcutoff = 50) %>% 
...
saveRDS(object = cybr2Data, file = "H2O2_Zeocin_CSS_cybr2.rds")
#getwd()
...
saveRDS(glm_fulldataset_8, file = "H2O2_CSS8_glm_SUB.rds")
...
rollData %>% select(-H2O2_Oak8_A_Wine) %>%
  pivot_longer(c(-CHROM, -POS), names_to = c("Bulk", "Parent", "Rep", "Allele"), names_sep = "_", values_to = "ReadCount") %>%
  filter(Bulk != "Zeocin") %>%
  mutate_if(is.character, as.factor) %>%
  #subsetting for time; this line can be removed if you want to see the whole genome
...
saveRDS(glm_fulldataset_8REP, file = "H2O2_MixedRepsChromosomes.rds")
...
saveRDS(glm_fulldataset_8REPex, file = "H2O2_8_ExclRep.rds")
...
saveRDS(glm_fulldataset_1, file = "H2O2_CSS1_glm.rds")
```
