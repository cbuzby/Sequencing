...
maybepeaks %>% select(-ppeakPOS) %>% pivot_wider(names_from = Pool, values_from = max_value) %>%
  ggplot(aes(x = HKTFTDRX2, y = HNGLVDRXY, label = CHROM)) + geom_label(aes(), alpha = 0.5) +
  geom_label(aes(y = HVYTYDRX2), color = "red") +
  geom_label(aes(y = HTG3TDMXY), color = "lightblue")
...
```{r}
CuSO4_All %>% filter(abs(Summary) > quant095) %>%
  pivot_wider(names_from = c(Pool, Dose), values_from = Summary) %>%
  ggplot(aes(x = HTG3TDMXY_High, y = HTG3TDMXY_Low)) + geom_point(alpha = 0.3)

CuSO4_All %>% filter(abs(Summary) > quant095) %>%
  filter(Dose == 0) %>%
  mutate(POS = round(POS/1000, 0)*1000) %>%
