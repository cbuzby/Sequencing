...
chrparentperm_c8 <- cybrPermute_byCHRParent_Rep(HVYTYDRX2_cu_CSS8_glm_prep)
...
#save these
saveRDS(chrparentperm_c8, file = "HVYTYDRX2_cu_CSS8_chrparentQs.rds")
saveRDS(chrparentperm_c8_ponly, file = "HVYTYDRX2_cu_CSS8_parQs.rds")
...
cybrPermute_byCHRParent <- function(dataset, R = FALSE){
   start.time <- Sys.time()
   ...
   if(R == TRUE){
    ...
    #these are now all of the ones that can be used to permute
    newnewtest %>% filter(Bulk == "aD") %>% select(CHROM, POS, Parent, Oak, Wine) %>% 
      group_by(CHROM, Parent) %>%
      summarize(CHROM = CHROM, Parent = Parent, Oak = Oak, Wine = Wine, POS = POS,
                POS2 = sample(POS)) %>%
      mutate(Bulk = "A") -> shuffled_DiluteA
    ...
    shuffletoglm %>% na.omit() %>% #filter(CHROM == "II") %>%
      #Original Script
      group_by(CHROM, POS) %>%
      mutate_if(is.character, as.factor) %>%
        summarize(Summary = glm_cb2_short(Allele = Allele,
...
cybrPermute_byParent <- function(dataset, R = FALSE){
  start.time <- Sys.time()
...
### HNGLVDRXY_CSS1 (CuSO4 CSS I A) - failed Feb 24
```{r}
...
### HKTMWDRX2_Zeo_CSS1_glm_prep_noreps (Zeocin and Cyclohex replicates averaged)

```{r}
HKTMWDRX2_Zeo_norep_CSS8_glm_prep_chrparent <- cybrPermute_byCHRParent(HKTMWDRX2_Zeo_CSS1_glm_prep_noreps)
HKTMWDRX2_Zeo_norep_CSS8_glm_prep_parentonly <- cybrPermute_byParent(HKTMWDRX2_Zeo_CSS1_glm_prep_noreps)

HKTMWDRX2_Zeo_norep_CSS8_glm_prep_chrparent %>% group_by(CHROM, Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HKTMWDRX2_Zeo_norep_CSS8_chrparentQs


HKTMWDRX2_Zeo_norep_CSS8_glm_prep_parentonly %>% group_by(Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HKTMWDRX2_Zeo_norep_CSS8_parQs

saveRDS(HKTMWDRX2_Zeo_norep_CSS8_chrparentQs, file = "StandardGLM/HKTMWDRX2_Zeo_norep_CSS8_chrparentQs.rds")
saveRDS(HKTMWDRX2_Zeo_norep_CSS8_parQs, file = "StandardGLM/HKTMWDRX2_Zeo_norep_CSS8_parQs.rds")
```

```{r}
HKTMWDRX2_Cyc_norep_CSS8_glm_prep_chrparent <- cybrPermute_byCHRParent(HKTMWDRX2_Cyc_CSS1_glm_prep_noreps)
HKTMWDRX2_Cyc_norep_CSS8_glm_prep_parentonly <- cybrPermute_byParent(HKTMWDRX2_Cyc_CSS1_glm_prep_noreps)

HKTMWDRX2_Cyc_norep_CSS8_glm_prep_chrparent %>% group_by(CHROM, Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HKTMWDRX2_Cyc_norep_CSS8_chrparentQs


HKTMWDRX2_Cyc_norep_CSS8_glm_prep_parentonly %>% group_by(Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HKTMWDRX2_Cyc_norep_CSS8_parQs

saveRDS(HKTMWDRX2_Cyc_norep_CSS8_chrparentQs, file = "StandardGLM/HKTMWDRX2_Cyc_norep_CSS8_chrparentQs.rds")
saveRDS(HKTMWDRX2_Cyc_norep_CSS8_parQs, file = "StandardGLM/HKTMWDRX2_Cyc_norep_CSS8_parQs.rds")
```
### Fluconazole CSSI without replicates - FIX THIS

This actually didn't work because there aren't any replicates and so each one just has a lot of NAs...

```{r}
HGVMVDRX2_CSS1_glm_prep <- readRDS("HGVMVDRX2_CSS1_glm_prep.rds") %>% mutate(Bulk = gsub("Dilute", "D", Bulk))
unique(HGVMVDRX2_CSS1_glm_prep$Bulk)

HGVMVDRX2_F_CSS1_glm_prep_chrparent <- cybrPermute_byCHRParent(HGVMVDRX2_CSS1_glm_prep, R = TRUE)
HGVMVDRX2_F_CSS1_glm_prep_parentonly <- cybrPermute_byParent(HGVMVDRX2_CSS1_glm_prep, R = TRUE)

HGVMVDRX2_F_CSS1_glm_prep_chrparent %>% group_by(CHROM, Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HGVMVDRX2_F_CSS1_chrparentQs


HGVMVDRX2_F_CSS1_glm_prep_parentonly %>% group_by(Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HGVMVDRX2_F_CSS1_parQs

saveRDS(HGVMVDRX2_F_CSS1_chrparentQs, file = "StandardGLM/HGVMVDRX2_F_CSS1_chrparentQs.rds")
saveRDS(HGVMVDRX2_F_CSS1_parQs, file = "StandardGLM/HGVMVDRX2_F_CSS1_parQs.rds")
```

### NEW: CuSO4 2024, no replicates
...
unique(HTG3TDMXY_CSS8_glm_prep$Bulk)

HTG3TDMXY_CSS8_chrparent <- cybrPermute_byCHRParent(HTG3TDMXY_CSS8_glm_prep)
HTG3TDMXY_CSS8_parentonly <- cybrPermute_byParent(HTG3TDMXY_CSS8_glm_prep)

HTG3TDMXY_CSS8_chrparent %>% group_by(CHROM, Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HTG3TDMXY_CSS8_glm_chrparentQs


HTG3TDMXY_CSS8_parentonly %>% group_by(Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HTG3TDMXY_CSS8_glm_parQs

saveRDS(HTG3TDMXY_CSS8_glm_chrparentQs, file = "StandardGLM/HTG3TDMXY_CSS8_glm_chrparentQs.rds")
saveRDS(HTG3TDMXY_CSS8_glm_parQs, file = "StandardGLM/HTG3TDMXY_CSS8_glm_parQs.rds")
```
...
HVYTYDRX2_cu_CSS8_glm_prep_chrparent <- cybrPermute_byCHRParent_Rep(HVYTYDRX2_cu_CSS8_glm_prep)
HVYTYDRX2_cu_CSS8_glm_prep_parentonly <- cybrPermute_byParent_Rep(HVYTYDRX2_cu_CSS8_glm_prep)

HVYTYDRX2_cu_CSS8_glm_prep_chrparent %>% group_by(CHROM, Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HVYTYDRX2_cu_CSS8_chrparentQs


HVYTYDRX2_cu_CSS8_glm_prep_parentonly %>% group_by(Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HVYTYDRX2_cu_CSS8_parQs

saveRDS(HVYTYDRX2_cu_CSS8_chrparentQs, file = "StandardGLM/HVYTYDRX2_cu_CSS8_chrparentQs.rds")
saveRDS(HVYTYDRX2_cu_CSS8_parQs, file = "StandardGLM/HVYTYDRX2_cu_CSS8_parQs.rds")
...
cybrPermute_byCHRParent_Rep6 <- function(dataset){
   ...
   print("Make sure that dilute bulk is labeled D")
    ...
    dataset %>% 
      ...
      ungroup() %>%
      distinct() %>% #THIS IS IMPORTANT
      pivot_wider(names_from = Allele, values_from = SmoothCount) %>% filter(Bulk == "aD") %>% select(CHROM, POS, Rep, #ADDED REP 1/8/24
                                                                                                     Parent, Oak, Wine) -> newnewtest
    
    #these are now all of the ones that can be used to permute
    # newnewtest %>% #filter(CHROM == "II") %>%
    #   group_by(CHROM, Parent) %>%
    #   summarize(CHROM = CHROM, Parent = Parent, Rep = Rep,
    #             Oak = Oak, Wine = Wine, POS = POS,
    #             POS2 = sample(POS)) %>%
    #   select(-POS) %>%
    #   group_by(CHROM, Parent, POS2) %>% 
    #   ## do the merge thing again
    #   summarize(CHROM = CHROM, Parent = Parent, Rep = Rep, 
    #             Oak = Oak, Wine = Wine, POS = POS2,
    #             culprits = length((Oak))) %>% 
    #   filter(culprits == 3) %>% 
    #   ungroup() %>%
...
# 
# #glmresult_1824 <- glmresult
# glmresult_1824 %>% ggplot(aes(y = abs(Summary), x = CHROM, color = Factor)) + geom_boxplot() + ylim(0,15)
# glmresult %>% ggplot(aes(y = abs(Summary), x = CHROM, color = Factor)) + geom_boxplot() + ylim(0,15)
...
## HTG3TDMXY CuSO4 2024 with replicates

```{r}
HTG3TDMXY_CSS8_glm_prep <- readRDS("StandardGLM/HTG3TDMXY_CSS8_glm_prep.rds") %>% 
  select(-Bulk) %>% 
  select(Bulk = B, Allele = Allele, POS = POS, Parent = Parent, SmoothCount = SmoothCount, Rep = Rep) %>% na.omit()  %>% filter(CHROM != "VIII", CHROM != "M")

unique(HTG3TDMXY_CSS8_glm_prep$Bulk)

HTG3TDMXY_CSS8_chrparentR6 <- cybrPermute_byCHRParent_Rep6(HTG3TDMXY_CSS8_glm_prep)
HTG3TDMXY_CSS8_parentonlyR6 <- cybrPermute_byCHRParent_Rep6(HTG3TDMXY_CSS8_glm_prep)

HTG3TDMXY_CSS8_chrparentR6 %>% group_by(CHROM, Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HTG3TDMXY_CSS8_chrparentR6_Qs


HTG3TDMXY_CSS8_parentonlyR6 %>% group_by(Factor) %>% 
  summarize(quant095 = quantile(abs(Summary), 0.95, na.rm = TRUE),
             quant099 = quantile(abs(Summary), 0.99, na.rm = TRUE),
             quant0995 = quantile(abs(Summary), 0.995, na.rm = TRUE)) -> HTG3TDMXY_CSS8_parentonlyR6_parQs

saveRDS(HTG3TDMXY_CSS8_chrparentR6_Qs, file = "StandardGLM/HTG3TDMXY_CSS8_chrparentR6_Qs.rds")
saveRDS(HTG3TDMXY_CSS8_parentonlyR6_parQs, file = "StandardGLM/HTG3TDMXY_CSS8_parentonlyR6_parQs.rds")
```