...
```{r}
HTG3TDMXY_CSS8_glm_prep <- readRDS("HTG3TDMXY_CSS8_glm_prep.rds") %>% 
  select(-Bulk) %>% 
...
cybrPermute_byCHRParent_Rep6 <- function(dataset){
   ...
   print("Make sure that dilute bulk is labeled D")
    ...
    dataset %>% 
      ...
      ungroup() %>%
      distinct() %>% #THIS IS IMPORTANT
      pivot_wider(names_from = Allele, values_from = SmoothCount) %>% filter(Bulk == "D") %>% select(CHROM, POS, Rep, #ADDED REP 1/8/24
