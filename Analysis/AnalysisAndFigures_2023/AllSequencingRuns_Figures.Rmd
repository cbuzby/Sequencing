...
```{r}
HTG3TDMXY_CSS8_glm_BULK %>% filter(Factor != "Intercept") %>%
  ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + geom_line() + 
  facet_grid(~CHROM, space = "free", scales = "free") + ggtitle("CSS8 b")

#This one's blank?
HVYTYDRX2_cu_CSS8_glm_BULK %>% filter(Factor != "Intercept") %>% na.omit()
  ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + geom_line() + 
  facet_grid(~CHROM, space = "free", scales = "free") + ggtitle("CSS8 a")

HKTFTDRX2_CSS1_glm_BULK %>% filter(Factor != "Intercept") %>%
  ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + geom_line() + 
  facet_grid(~CHROM, space = "free", scales = "free") + ggtitle("CSS1 b")

HNGLVDRXY_CSS1_glm_BULK %>% filter(Factor != "Intercept") %>%
  ggplot(aes(x = POS, y = abs(Summary), color = Factor)) + geom_line() + 
