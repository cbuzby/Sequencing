...
#compare chromosome x parent cutoffs
chrparentperm_c8 %>% ungroup() %>% group_by(CHROM, Factor) %>% 
  summarize(quant = quantile(abs(Summary), 0.95, na.rm = TRUE)) %>% mutate(ds = "function_chrom") -> function_chrom
cu8_cutoffs %>% ungroup() %>% group_by(CHROM, Factor) %>% 
  summarize(quant = quantile(abs(Summary), 0.95, na.rm = TRUE)) %>% mutate(ds = "nonfunction_chrom") -> nonfunction_chrom

#compare parent only cutoffs
chrparentperm_c8_ponly %>% ungroup() %>% group_by(Factor) %>% 
  summarize(quant = quantile(abs(Summary), 0.95, na.rm = TRUE)) %>% mutate(ds = "function_parent") -> function_parent
cu8_cutoffs_2 %>% ungroup() %>%  group_by(Factor) %>% 
  summarize(quant = quantile(abs(Summary), 0.95, na.rm = TRUE)) %>% mutate(ds = "nonfunction_parent") -> nonfunction_parent
...
```{r, fig.width = 10, fig.height = 4}
rbind(function_chrom, nonfunction_chrom) %>% ggplot(aes(y = quant, x = CHROM, color = ds)) + geom_boxplot() 

rbind(function_parent, nonfunction_parent) %>% ggplot(aes(y = ds, x = quant, color = Factor)) + geom_jitter()
```
