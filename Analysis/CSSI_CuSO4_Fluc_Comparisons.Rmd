---
title: "CSS I Fluconazole vs CuSO4 Experiments"
...
CompareNulls <- function(Table1 = CSSI_Fluc_Pivot, Table2 = Table1, D1 = "WineI_Dilute_B", D2 = "WineI_Dilute_C"){
  Table1 %>% filter(Dataset == D1) -> WD
  str(WD)
  #Table2 %>% filter(Dataset == D2) -> test
  str(test)
  #%>% rbind(WD) -> WineDilute
...
unique(CSSI_Fluc_Pivot$Dataset)
unique(CSSI_CuSO4_Pivot$Dataset)

CSSI_CuSO4_Pivot %>% mutate(Day = 0) -> CSSI_CuSO4_Pivot_D
CuF_Oak_Dilute <- CompareNulls(Table1 = CSSI_Fluc_Pivot,
                                Table2 = CSSI_CuSO4_Pivot_D,
                                D1 = "OakI_Dilute_B",
                                D2 = "OakI_CuSO4_D")
save(CuF_Oak_Dilute, file = "RdataFiles/CuF_Oak_Dilute.Rdata")

#CSSI_CuSO4_Pivot %>% mutate(Day = 0) -> CSSI_CuSO4_Pivot_D
CuF_Wine_Dilute <- CompareNulls(Table1 = CSSI_Fluc_Pivot,
                                Table2 = CSSI_CuSO4_Pivot_D,
                                D1 = "WineI_Dilute_B",
                                D2 = "WineI_CuSO4_D")
save(CuF_Wine_Dilute, file = "RdataFiles/CuF_Wine_Dilute.Rdata")

CuF_Wine_DiluteC <- CompareNulls(Table1 = CSSI_Fluc_Pivot, 
                                Table2 = CSSI_CuSO4_Pivot_D,
                                D1 = "WineI_Dilute_C", 
                                D2 = "WineI_CuSO4_D")
save(CuF_Wine_DiluteC, file = "RdataFiles/CuF_Wine_DiluteC.Rdata")

#CSSI_CuSO4_Pivot %>% mutate(Day = 0) -> CSSI_CuSO4_Pivot_D
CuF_Wine_DiluteD <- CompareNulls(Table1 = CSSI_Fluc_Pivot, 
                                Table2 = CSSI_CuSO4_Pivot_D,
                                D1 = "WineI_Dilute_D", 
                                D2 = "WineI_CuSO4_D")
save(CuF_Wine_DiluteD, file = "RdataFiles/CuF_Wine_DiluteD.Rdata")

#Reversing oak and wine to test
CuF_OakB_Wine <- CompareNulls(Table1 = CSSI_Fluc_Pivot, 
                                Table2 = CSSI_CuSO4_Pivot_D,
                                D1 = "OakI_Dilute_B", 
                                D2 = "WineI_CuSO4_D")
save(CuF_OakB_Wine, file = "RdataFiles/CuF_OakB_Wine.Rdata")

```


Combining these replicates of only CSS I Fluconazole experiment
...
Combining these replicates of the two experiments

```{r}
load("RdataFiles/CuF_Oak_Dilute.Rdata")
load("RdataFiles/CuF_Wine_Dilute.Rdata")
load("RdataFiles/CuF_Wine_DiluteC.Rdata")
load("RdataFiles/CuF_Wine_DiluteD.Rdata")
load("RdataFiles/CuF_OakB_Wine.Rdata")


CuF_Oak_Dilute$Comparison <- "Oak_FlucvsCuSO4"
CuF_Wine_Dilute$Comparison <- "Wine_FlucvsCuSO4_B"
CuF_Wine_DiluteC$Comparison <- "Wine_FlucvsCuSO4_C"
CuF_Wine_DiluteD$Comparison <- "Wine_FlucvsCuSO4_D"
CuF_OakB_Wine$Comparison <- "FOakB_CWine"

CF_NullComparison <- rbind(CuF_Oak_Dilute,CuF_Wine_Dilute, CuF_Wine_DiluteC, CuF_Wine_DiluteD, CuF_OakB_Wine)

CF_NullComparison %>% ggplot(aes(x = POS, y = Dataset_Zprime, color = Comparison)) + geom_line(alpha = 0.8) + 
  scale_x_continuous(breaks = seq(from = 0, to = max(CF_NullComparison$POS), by = 1e5), name = "Genomic Position") +
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + theme_minimal() +
  geom_hline(yintercept = c(-1.96, 1.96), color = "gray") + 
  scale_color_manual(values = c("black", "navyblue", "red", "firebrick", "darkorange")) +
  ggtitle("All Null Replicate Comparisons") + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank()) + 
  xlab("Genomic Position") + ylab("Oak  <--    Z Score  -->  Wine")

CF_NullComparison %>% filter(CHROM != "I") %>% ggplot(aes(x = POS, y = Dataset_Zprime, color = Comparison)) + geom_line(alpha = 0.8) + 
  scale_x_continuous(breaks = seq(from = 0, to = max(CF_NullComparison$POS), by = 1e5), name = "Genomic Position") +
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + theme_minimal() +
  geom_hline(yintercept = c(-1.96, 1.96), color = "gray") + 
  scale_color_manual(values = c("black", "navyblue", "red", "firebrick", "darkorange")) +
  ggtitle("All Null Replicate Comparisons w/o Chr I") + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(), legend.position = "null") + 
  xlab("Genomic Position") + ylab("Oak  <--    Z Score  -->  Wine")

```

Compare just the oak and wine dilutes from the original experiment

```{r}
# CuSO4_Null_Comparison <- CompareNulls(Table1 = CSSI_CuSO4_Pivot_D, 
#                                 Table2 = CSSI_CuSO4_Pivot_D,
#                                 D1 = "OakI_CuSO4_D", 
#                                 D2 = "WineI_CuSO4_D")
# save(CuSO4_Null_Comparison, file = "RdataFiles/CuSO4_Null_Comparison.Rdata")

load("RdataFiles/CuSO4_Null_Comparison.Rdata")

CuSO4_Null_Comparison %>% ggplot(aes(x = POS, y = Dataset_Zprime)) + geom_line() + 
  scale_x_continuous(breaks = seq(from = 0, to = max(CuSO4_Null_Comparison$POS), by = 1e5), name = "Genomic Position") +
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + theme_minimal() +
  geom_hline(yintercept = c(-1.96, 1.96), color = "gray") + 
  ggtitle("All Null Replicate Comparisons") + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank()) + 
  xlab("Genomic Position") + ylab("Oak  <--    Z Score  -->  Wine")
...
```
## Adding lines from comparisons of replicates

```{r}

CuSO4_Null_Comparison %>% filter(CHROM != "I") %>% select(CHROM, Dataset_Zprime) -> CuSO4_cutoff_df
NullComparison %>% filter(CHROM != "I") %>% select(CHROM, Dataset_Zprime) -> Fluconazole_cutoff_df

C_cutoff <- quantile(abs(CuSO4_cutoff_df$Dataset_Zprime), 0.975)
F_cutoff <- quantile(abs(Fluconazole_cutoff_df$Dataset_Zprime), 0.975)

CuSO4_cutoff_df %>% ggplot(aes(x = Dataset_Zprime)) + 
  geom_density(fill = "blue", alpha = 0.4, size = 1) + 
  geom_vline(xintercept = c(C_cutoff, -C_cutoff)) +
    geom_vline(xintercept = c(max(abs(CuSO4_cutoff_df$Dataset_Zprime)), -max(abs(CuSO4_cutoff_df$Dataset_Zprime))), linetype = "dashed") +
  xlim(-3, 3) + 
  ylim(0,2) + 
  ggtitle("CuSO4 Nulls")

Fluconazole_cutoff_df %>% ggplot(aes(x = Dataset_Zprime)) + 
  geom_density(fill = "goldenrod", alpha = 0.4, size = 1) + 
      geom_vline(xintercept = c(max(abs(Fluconazole_cutoff_df$Dataset_Zprime)), -max(abs(Fluconazole_cutoff_df$Dataset_Zprime))), linetype = "dashed") +
  geom_vline(xintercept = c(F_cutoff, -F_cutoff))+  
  xlim(-3, 3) + 
  ylim(0,2) + ggtitle("Fluconazole Replicates")
...
CSSI_C_ZScores_GLM_noday %>% #filter(CHROM != "I") %>% 
  ...
  ggtitle("CSSI CuSO4 | Comparison of Smoothed Z scores") + theme(legend.position = "none", axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + 
  ylim(-10, 10) + 
  geom_hline(yintercept = c(C_cutoff, -C_cutoff)) +
  geom_hline(yintercept = c(max(abs(CuSO4_cutoff_df$Dataset_Zprime)), -max(abs(CuSO4_cutoff_df$Dataset_Zprime))), linetype = "dashed") +
  xlab("Genomic Position") + ylab("Oak  <--    Z Score  -->  Wine")
...
CSSI_F_ZScores_GLM %>% #filter(CHROM != "I") %>% 
  ...
  geom_point(aes(x = POS, y = Interaction_Zprime), color = "#D7335C", alpha= 0.8, size = 0.2) +
    geom_point(aes(x = POS, y = Day_Zprime), color = "gray", alpha= 0.8, size = 0.2) +
  geom_hline(yintercept = c(max(abs(Fluconazole_cutoff_df$Dataset_Zprime)), -max(abs(Fluconazole_cutoff_df$Dataset_Zprime))), linetype = "dashed") +
  scale_x_continuous(breaks = seq(from = 0, to = max(CSSI_F_ZScores_GLM$POS), by = 1e5), name = "Genomic Position") +
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + theme_minimal() +
  ggtitle("CSSI Fluconazole | Comparison of Smoothed Z scores") + theme(legend.position = "none", axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + 
  ylim(-10, 10) + geom_hline(yintercept = c(F_cutoff, -F_cutoff)) +
  xlab("Genomic Position") + ylab("Oak  <--    Z Score  -->  Wine")
```

## Make the individual plots for CuSO4

```{r}

load("RdataFiles/CSSI_CuSO4_Pivot_exclmD.Rdata")
CSSI_CuSO4_Pivot_exclmD

CSSI_CuSO4_Pivot_exclmD$Bulk <- factor(CSSI_CuSO4_Pivot_exclmD$Bulk, levels = c("Control", "Dilute", "CuSO4", "Fluconazole"))
CSSI_CuSO4_Pivot_exclmD$Bulk[CSSI_CuSO4_Pivot_exclmD$Bulk == "Dilute"] <- "Control"
unique(CSSI_CuSO4_Pivot_exclmD$Bulk)

for(i in unique(CSSI_CuSO4_Pivot_exclmD$CHROM)){
  plot <- CSSI_CuSO4_Pivot_exclmD %>% filter(CHROM == i) %>% select(-ALT_REF) %>% 
    pivot_wider(names_from = Allele, values_from = ReadCount) %>% 
    ggplot(aes(x = POS, y = log(Wine/Oak), color = Dataset)) + 
    geom_point(size = 1, alpha = 0.4) + 
    #geom_line(alpha = 0.2) +
    scale_color_manual(values = c("navyblue", "blue", "firebrick", "red")) +
    ggtitle(i) + theme(legend.position = "blank") +
    facet_grid(~Bulk+Parent) 
  print(plot)
}

plot_c <- CSSI_CuSO4_Pivot_exclmD %>% filter(CHROM == "VII") %>% select(-ALT_REF) %>% 
    pivot_wider(names_from = Allele, values_from = ReadCount) %>% 
    ggplot(aes(x = POS, y = log(Wine/Oak), color = Dataset)) + 
    geom_point(size = 1, alpha = 0.4) + 
    #geom_line(alpha = 0.2) +
    scale_color_manual(values = c("navyblue", "blue", "firebrick", "red")) +
    ggtitle("CuSO4 Experiment Chr VII") + theme(legend.position = "blank") +
    facet_grid(~Bulk+Parent)

plot_c

```
Make this plot for Fluconazole

```{r}
load("RdataFiles/CSSI_Fluc_Pivot_exclmD.Rdata")
CSSI_Fluc_Pivot_exclmD

CSSI_Fluc_Pivot_exclmD$Bulk <- factor(CSSI_Fluc_Pivot_exclmD$Bulk, levels = c("Control", "Dilute", "CuSO4", "Fluconazole"))
CSSI_Fluc_Pivot_exclmD$Bulk[CSSI_Fluc_Pivot_exclmD$Bulk == "Dilute"] <- "Control"

unique(CSSI_Fluc_Pivot_exclmD$Dataset)

for(i in unique(CSSI_Fluc_Pivot_exclmD$CHROM)){
  plot <- CSSI_Fluc_Pivot_exclmD %>% filter(Dataset == "OakI_Dilute_B" | Dataset == "OakI_Fluc_B" | Dataset == "WineI_Dilute_B" | Dataset == "WineI_Fluc_D") %>% filter(CHROM == i) %>% select(-ALT_REF) %>% 
    pivot_wider(names_from = Allele, values_from = ReadCount) %>% 
    ggplot(aes(x = POS, y = log(Wine/Oak), color = Dataset)) + 
    geom_point(size = 1, alpha = 0.4) + 
    #geom_line(alpha = 0.2) +
    scale_color_manual(values = c("blue", "navyblue", "red", "firebrick")) +
    ggtitle(i) + 
    facet_grid(~Bulk+Parent) 
  print(plot)
}

plot_f <- CSSI_Fluc_Pivot_exclmD %>% filter(Dataset == "OakI_Dilute_B" | Dataset == "OakI_Fluc_B" | Dataset == "WineI_Dilute_B" | Dataset == "WineI_Fluc_D") %>% filter(CHROM == "VII") %>% select(-ALT_REF) %>% 
    pivot_wider(names_from = Allele, values_from = ReadCount) %>% 
    ggplot(aes(x = POS, y = log(Wine/Oak), color = Dataset)) + 
    geom_point(size = 1, alpha = 0.4) + 
    #geom_line(alpha = 0.2) +
    scale_color_manual(values = c("blue", "navyblue", "red", "firebrick")) +
    ggtitle("Fluconazole Experiment Chr VII") + theme(legend.position = "blank") +
    facet_grid(~Bulk+Parent) 

plot_f
```

Finding a good example of the table

```{r}
CSSI_Fluc_Pivot_exclmD[CSSI_Fluc_Pivot_exclmD$POS > 3000 & CSSI_Fluc_Pivot_exclmD$POS < 5000 & CSSI_Fluc_Pivot_exclmD$CHROM == "VII",]
```

Find Peaks for Interactions
...
CSSI_F_ZScores_GLM %>% filter(CHROM == "VII") -> ChrVII_Peaks_F

ChrVII_Peaks_F[ChrVII_Peaks_F$Bulk_Zprime == max(ChrVII_Peaks_F$Bulk_Zprime),]
ChrVII_Peaks_F[ChrVII_Peaks_F$Bulk_Zprime == min(ChrVII_Peaks_F$Bulk_Zprime),]
