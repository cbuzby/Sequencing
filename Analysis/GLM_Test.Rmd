...
rm(Results_II, Results_III, Results_IV, Results_V, Results_VI, Results_VII, Results_VIII, Results_IX, Results_X, Results_XI, 
      Results_XII, Results_XIII, Results_XIV, Results_XV, Results_XVI)
...
windowresults %>% filter(CHROM == "XII") %>% 
  transmute(CHROM = CHROM, POS = POS, Bulk = Zprime_Bulk, Parent = Zprime_Parent, Int = Zprime_Int, Stat = "ZPrime") -> XIIZprime
...
Res_Z_Chr %>% transmute(CHROM = CHROM, POS = V2, Bulk = bulkHigh, Parent = parentWine, Int = Interaction, Stat = "Z") -> Zs
...
## Is there actually a correlation between chromosomes with large effects and those with higher int scores?

```{r}
betaresults %>% group_by(CHROM) %>% summarise(maxBulk = max(abs(Zprime_Bulk)),
                                              maxParent = max(abs(Zprime_Parent)),
                                              maxInt = max(abs(Zprime_Int))) -> betamax


plot(betamax$maxBulk, betamax$maxInt)
plot(betamax$maxParent, betamax$maxInt)
plot(betamax$maxBulk, betamax$maxParent)

#alright, it doesn't look like there's any correlation here, so that at least is good
...
First, what's the z-score at a p-value of 0.05? Then what's the z-score at much lower than that?

```{r}
pnorm(q = c(-0.05, -0.5, -5))

dnorm(x = c(-0.05, -0.5, -5))
```
