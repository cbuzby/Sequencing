...
library(ggcorrplot)
load("RdataFiles/CSSI_Fluc_Pivot.Rdata")
...
```{r}
load("RdataFiles/CSSI_Fluc_Pivot_exclmD.Rdata")
...
zscore_1 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "I")
zscore_2 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "II")
zscore_3 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "III")
zscore_4 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "IV")
Sys.sleep(5)

zscore_5 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "V")
zscore_6 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "VI")
zscore_7 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "VII")
zscore_8 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "VIII")
Sys.sleep(5)

zscore_9 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "IX")
zscore_10 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "X")
zscore_11 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "XI")
zscore_12 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "XII")
Sys.sleep(5)

zscore_13 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "XIII")
zscore_14 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "XIV")
zscore_15 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "XV")
zscore_16 <- BSA_GLM_CSSIF_noDay(CSSI_Fluc_Pivot, "XVI")
Sys.sleep(5)
...
save(CSSI_F_ZScores_GLM_noday, file = "CSSI_F_ZScores_GLM_noday2.Rdata")
```

```{r}
 
load("RdataFiles/CSSI_F_ZScores_GLM_noday2.Rdata")
...
head(CSSI_F_Wine_BC_Dilute_ZScores_GLM)
...
## Calculating the FDR using adjusted p-values

1. Rank your p-values
2. Assign a q-value(?)
3. What is the p-value when the q-values are a certain number??????
...
CSSI_F_ZScores_GLM %>% arrange(abs(Bulk_Zprime)) %>% mutate(Bulk_Zprime_Rank = length(Bulk_Zprime):1) %>%
  arrange(abs(Parent_Zprime)) %>% mutate(Parent_Zprime_Rank = length(Bulk_Zprime):1) %>%
  arrange(abs(Day_Zprime)) %>% mutate(Day_Zprime_Rank = length(Bulk_Zprime):1) %>%
  arrange(abs(Interaction_Zprime)) %>% mutate(Interaction_Zprime_Rank = length(Bulk_Zprime):1) -> ranks

ggplot(ranks, aes(x = Bulk_Zprime_Rank, y = abs(Bulk_Zprime))) + geom_line(color = "#345F6F", size = 1) +
  geom_line(aes(x = Parent_Zprime_Rank, y = abs(Parent_Zprime)), color = "#FFB05C", size = 1) +
  geom_line(aes(x = Day_Zprime_Rank, y = abs(Day_Zprime)), color = "gray", size = 1) +
  geom_line(aes(x = Interaction_Zprime_Rank, y = abs(Interaction_Zprime)), color = "#D7335C", size = 1) +
  ggtitle("Ranked Z-scores by Factor") + ylab("Absolute Value of Z-Score") + xlab("Rank") +
  geom_hline(yintercept = c(1.96,1), linetype = "dashed")

#Testing formulas for below function
# qnorm(.025)
# pnorm(-1.96, lower.tail = T)*2

CSSI_F_ZScores_GLM %>% select(-Bulk_Z, -Parent_Z, -Interaction_Z, -Day_Z) %>% 
  pivot_longer(c(Bulk_Zprime, Parent_Zprime, Interaction_Zprime, Day_Zprime), names_to = "Effect", values_to = "Zprime") %>%
  arrange(abs(Zprime)) %>% mutate(Rank = length(Effect):1, pval = pnorm(abs(Zprime), lower.tail = F)*2) -> TotalRanks

qvalresults <- qvalue(TotalRanks$pval, fdr.level = 0.5)
TotalRanks$qval <- qvalresults$qvalues

#head(qvalresults)

# ggplot(TotalRanks, aes(x = Rank, y = abs(Zprime), color = Effect)) + geom_point(alpha = 0.4) + scale_color_manual(values = c("#345F6F", "gray", "#D7335C", "#FFB05C"))
# 
# ggplot(TotalRanks, aes(x = Rank, y = abs(Zprime), color = Effect)) + geom_line(size = 1) + scale_color_manual(values = c("#345F6F", "gray", "#D7335C", "#FFB05C")) + facet_wrap(~Effect) + geom_hline(yintercept = 1, linetype = "dashed") 
# 
# ggplot(TotalRanks, aes(x = Rank, y = pval, color = Effect)) + geom_line(size = 1) + scale_color_manual(values = c("#345F6F", "gray", "#D7335C", "#FFB05C")) + facet_wrap(~Effect) + geom_hline(yintercept = 0.05, linetype = "dashed") 

ggplot(TotalRanks, aes(y = qval, x = Zprime, color = Effect)) + geom_line(size = 1) + scale_color_manual(values = c("#345F6F", "gray", "#D7335C", "#FFB05C")) + geom_hline(yintercept = 0.05, linetype = "dashed") + facet_wrap(~Effect)

ggplot(TotalRanks, aes(x = POS, y = -log(qval), color = Effect)) + geom_line(size = 1) +
  scale_color_manual(values = c("#345F6F", "gray", "#D7335C", "#FFB05C"))+ 
  facet_grid(~CHROM, scales = "free_x", space = "free_x") +
  theme_minimal() + theme(legend.position = "none", axis.text.x=element_blank(),axis.ticks.x=element_blank())
```

## Using "Population" for different "Day" values so it's just one of each?

This doesn't help at all, and makes everything confusing because each population then is its own thing and that takes up all of the effects.

```{r, eval = FALSE}
load("RdataFiles/CSSI_Fluc_Pivot_2.Rdata")
CSSI_Fluc_Pivot$Day <- CSSI_Fluc_Pivot$Dataset
...
CSSI_F_ZScores_GLM_DATASET <- rbind(zscore_1, zscore_2, zscore_3, zscore_4, zscore_5, zscore_6, 
                     zscore_7, zscore_8, zscore_9, zscore_10, zscore_11, 
                     zscore_12, zscore_13, zscore_14, zscore_15, zscore_16)

CSSI_F_ZScores_GLM_DATASET$CHROM <- factor(CSSI_F_ZScores_GLM_DATASET$CHROM, levels = ChromKey$chromosomes)

save(CSSI_F_ZScores_GLM_DATASET, file = "RdataFiles/CSSI_F_ZScores_GLM_DATASET.Rdata")
```



```{r, eval = FALSE}
#Does the day matter now? THIS IS NOT USEFUL AT ALL

load("RdataFiles/CSSI_F_ZScores_GLM_DATASET.Rdata")

CSSI_F_ZScores_GLM_DATASET %>% ggplot(aes(x = POS, y = Bulk_Zprime)) + 
      # geom_vline(data = JaroszKey, aes(xintercept = POS), color = "black", alpha = 0.5, size = 0.7) +
      # geom_vline(data = InteractionNetwork, aes(xintercept = POS), color = "red", alpha = 1, size = 0.7, linetype = "dashed") +
      # 
  geom_point(color = "#345F6F", size = 0.2, alpha = 0.8) + 
  ...
  geom_point(aes(x = POS, y = Interaction_Zprime), color = "#D7335C", alpha= 0.8, size = 0.2) +
  geom_point(aes(x = POS, y = Day_Zprime), color = "gray", alpha= 0.8, size = 0.2) +
  scale_x_continuous(breaks = seq(from = 0, to = max(CSSI_F_ZScores_GLM_DATASET$POS), by = 1e5), name = "Genomic Position") +
  facet_grid(~CHROM, scales = "free_x", space = "free_x") + theme_minimal() +
  geom_hline(yintercept = c(max(abs(NullComparison$Dataset_Zprime)), -max(abs(NullComparison$Dataset_Zprime))), 
             color = "black", linetype = "dashed") +
  ggtitle("CSSI Fluconazole | Comparison of Smoothed Z scores") + theme(legend.position = "none", axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + ylim(-10, 10) +
  xlab("Genomic Position") + ylab("Oak  <--    Z Score  -->  Wine")

```

Looking at cutoffs of subsetting and comparing between nulls
...
### Plot using the between 95% cutoff
...
CSSI_F_ZScores_GLM_noday %>% ggplot(aes(x = POS, y = Bulk_Zprime)) + 
